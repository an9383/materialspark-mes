...
<script>
	...
	let menuAuth = 'wmsc2210';
	let currentHref = 'wmsc2210';
    ...
    $('#btnOption1').on('click', function() {
   		...
   		$.ajax({
       		url: '<c:url value="wm/readMonthPlanBodySLDCheckMTP2"/>',
   			type: 'POST',  
   			async: false,
   			data: {
   				'menuAuth'	:	menuAuth,
    ...
    $('#btnComplete').on('click', function() {  
    	...
    	$.ajax({
    		url: '<c:url value="wm/updateCompleteYnMTP2"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
...
/* 		$.each(npGubunList,function(index,item){
    ...
    function monthProductPresentTableFunc(){
        ...
        let tempTableData = $('#productionPlanTable').on( 'error.dt', function ( e, settings, techNote, message ) {
        		...
        		ajax: {
                    url: '<c:url value="/wm/readMonthPlanBodySLDMTP2"/>',
                    type: 'POST',
                    data: {
                    	'menuAuth'		:		menuAuth,
                    	'fpDate'		:		function(){return inWhsDateCal.replace(/-/g,"");},
    ...
    $('#btnYeildSave').on('click', function() {
    	$.ajax({
			url: '<c:url value="wm/monthProductionPlanYeildCreateMTP2"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
