...
<script>
	...
	let menuAuth = 'wmsc1210';
	let currentHref = 'wmsc1210';
    ...
    $('#btnOption1').on('click', function() {
   		...
   		$.ajax({
       		url: '<c:url value="wm/readMonthPlanBodySLDCheckMTP"/>',
   			type: 'POST',  
   			async: false,
   			data: {
   				'menuAuth'	:	menuAuth,
    ...
    $('#btnComplete').on('click', function() {  
    	...
    	$.ajax({
    		url: '<c:url value="wm/updateCompleteYnMTP"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
...
/* 		$.each(npGubunList,function(index,item){
    ...
    function monthProductPresentTableFunc(){
        ...
        let tempTableData = $('#productionPlanTable').on( 'error.dt', function ( e, settings, techNote, message ) {
        		...
        		ajax: {
                    url: '<c:url value="/wm/readMonthPlanBodySLDMTP"/>',
                    type: 'POST',
                    data: {
                    	'menuAuth'		:		menuAuth,
                    	'fpDate'		:		function(){return inWhsDateCal.replace(/-/g,"");},
    ...
    $('#btnYeildSave').on('click', function() {
    	$.ajax({
			url: '<c:url value="wm/monthProductionPlanYeildCreateMTP"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
