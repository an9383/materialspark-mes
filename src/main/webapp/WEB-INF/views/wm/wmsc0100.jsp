	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">월별 FP현황(설비별)</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
			...
			<div class="left-list" id="left-list">
				...
				<div class="row">
					<div class="row col-sm-6" style="margin-bottom:5px;">
						<!-- <label class="input-label-sm mr-1">계획 월&일자</label> -->
						<!-- <input type="text" class="form-control mr-2" id="inWhsDateCal" style="width:65px;"></input> -->
						<label class="input-label-sm">계획 월&일자</label>
						<div class="form-group input-sub m-0 row">
							<input class="form-control" style="width:97px;" type="text" id="inWhsDateCal" name="inWhsDateCal" />
							<button onclick="fnPopUpCalendar(inWhsDateCal,inWhsDateCal, 'yyyy-mm-dd')"  class="btn btn-secondary input-sub-search" id="chooseDate1Cendar" type="button">
								<span class="oi oi-calendar"></span>
							</button>
						</div>
	    				<button type="button" class="btn btn-primary float-left ml-2" id="btnRetv">조회</button>
	    			</div>
	    		</div>
				<div class="card-header card-tab">
					<ul class="nav nav-tabs card-header-tabs">
						<li class="nav-item"><a class="nav-link active show" data-toggle="tab" id="tab1">메인</a></li>
						<li class="nav-item"><a class="nav-link" data-toggle="tab" id="tab2">요약</a></li>
					</ul>
				</div>			
				<div id="View1">	
					<div class="second-table">
						<table id="monthProductPresentTable2" class="table table-bordered">
							<thead class="thead-light">
								<tr id="planHeader2"></tr>
							</thead>
						</table>
					</div>
				...
				<div id="View2" class="d-none" >
					<div class="third-table">
						<table id="monthProductPresentTable3" class="table table-bordered">
							<thead class="thead-light">
								<tr id="planHeader3"></tr>
							</thead>
						</table>
					</div>
				</div>
...
<script>
	...
	$(document).attr("title","월별 FP현황(설비별)");

	$('#my-spinner').show();
	
    var chooseDate =  "${yesterDate}";
	//chooseDate = chooseDate.substring(0,7);
	var temp = true;
	var sum1 = 0;
	var sum2 = 0;
	var sum3 = 0;
	...
	$.fn.dataTable.ext.errMode = 'none';
	let monthProductPresentTableOrigin2 = $('#monthProductPresentTable2').parent().html();
	let monthProductPresentTable2 = monthProductPresentTableFunc2();

	$.fn.dataTable.ext.errMode = 'none';
	let monthProductPresentTableOrigin3 = $('#monthProductPresentTable3').parent().html();
	let monthProductPresentTable3 = monthProductPresentTableFunc3();
	

	
    	
	//var html1 =  '<div class="row">&nbsp;<label class="input-label-sm">계획 월</label><input type="text" class="form-control mr-1" id="chooseDate" style="width:65px;">';
    //	html1 += '&nbsp; <button type="button" class="btn btn-primary" id="btnRetv">조회</button></div>';
	//$('#purchaseOutputInfoTable_length').html(html1);
	$('#inWhsDateCal').val(chooseDate);
	...
	function createHeader(){
		return new Promise((resolve, reject)=> {
			...
			$.ajax({
				...
				data: {
					'menuAuth'	:	menuAuth,
					'planDate'	:	function(){return chooseDate.replace(/-/g,"");},
				},
    ...
    function monthProductPresentTableFunc(){
        ...
        let tempTableData = $('#monthProductPresentTable').on( 'error.dt', function ( e, settings, techNote, message ) {
    		...
    		ajax: {
                url: '<c:url value="/wm/readMonthProductPresentBody"/>',
                type: 'POST',
                data: {
                	'menuAuth'		:		menuAuth,
                	'planDate'		:		function(){return chooseDate.replace(/-/g,"");},
    		...
    		drawCallback:function(){
    			...
    			$.each($('#monthProductPresentTable tbody tr'),function(index, item, array){
    				if(index < 6){
    				...
    				if($('.day31').text()==""){
    					$(this).find('th').eq(35).addClass('d-none');
    					$(this).find('td').eq(35).addClass('d-none');
    				}

    				if(index > 5){
    					if($(this).find('td').eq(0).text() != equipCd){
    						equipCd = $(this).find('td').eq(0).text();
    						colorStat = !colorStat;
    			...
    			$('#monthProductPresentTable').rowspan(0);
    			//$('#monthProductPresentTable').rowspan(1);
    			//$('#monthProductPresentTable').rowspan(2);
    ...
    function monthProductPresentTableFunc2(){
        //console.log("난 테이블이야")
        let tempTableData = $('#monthProductPresentTable2').on( 'error.dt', function ( e, settings, techNote, message ) {
    		if(techNote == 7) {
    			toastr.error("로그인 세션이 만료 되었습니다.<br/>재로그인 해 주세요.", '', {timeOut: 5000});
    			location.href = "/";
    		}
    		...
    		ajax: {
                url: '<c:url value="/wm/readMonthProductPresentBodyOnlyHeader"/>',
                type: 'POST',
                data: {
                	'menuAuth'		:		menuAuth,
                	'planDate'		:		function(){return chooseDate.replace(/-/g,"");},
    		...
    		drawCallback:function(){
    			...
    			var equipCd = '';
    			$('.col-sm-12').css("padding", "0px");
    			$.each($('#monthProductPresentTable2 tbody tr'),function(index, item, array){    			
    				if($('.day27').text()==""){
    					$(this).find('th').eq(31).addClass('d-none');
    					$(this).find('td').eq(31).addClass('d-none');
    				}
                    ...
                    if(index == $('#monthProductPresentTable2').DataTable().data().count() - 1){
    			...
    			$('#monthProductPresentTable2').rowspan(0);
    			//$('#monthProductPresentTable').rowspan(1);
    			//$('#monthProductPresentTable').rowspan(2);
	...
	function createHeader3(){
	   ...
	   function monthProductPresentTableFunc3(){
	        //console.log("난 테이블이야")
	        let tempTableData = $('#monthProductPresentTable3').on( 'error.dt', function ( e, settings, techNote, message ) {
	    		if(techNote == 7) {
	    			toastr.error("로그인 세션이 만료 되었습니다.<br/>재로그인 해 주세요.", '', {timeOut: 5000});
	    			location.href = "/";
	    		}
	    		...
	    		drawCallback:function(){
	    			...
	    			var equipCd = '';
	    			$('.col-sm-12').css("padding", "0px");
	    			$('#monthProductPresentTable3').rowspan2(0);
	    			$.each($('#monthProductPresentTable3 tbody tr'),function(index, item){
	    				if($('.day27').text()==""){
	    				...
	    				if(index == $('#monthProductPresentTable3').DataTable().data().count() - 1){
	                    	$('#my-spinner').hide();
	                    }    		
