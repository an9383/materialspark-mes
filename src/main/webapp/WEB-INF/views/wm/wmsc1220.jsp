...
<script>
	...
	let menuAuth = 'wmsc1220';
	let currentHref = 'wmsc1220';
    ...
    $('#btnEdit').on('click', function() {        	
    	...
    	$.ajax({
    		url: '<c:url value="wm/checkCompleteYnMTP"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
    ...
    $('#btnComplete').on('click', function() {  
    	...
    	$.ajax({
    		url: '<c:url value="wm/updateCompleteYnModifyMTP"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
	...
	$('#btnSave').on('click', function() {
		...
		$.ajax({
            url: '<c:url value="/wm/insertMonthProductionPlanMTP"/>',
            type: 'POST',
...
/* 		$.each(npGubunList,function(index,item){
    ...
    function monthProductPresentTableFunc(){
        ...
        let tempTableData = $('#productionPlanTable').on( 'error.dt', function ( e, settings, techNote, message ) {
        		...
        		ajax: {
                    url: '<c:url value="/wm/readMonthPlanBodySLDModifyMTP"/>',
                    type: 'POST',
                    data: {
                    	'menuAuth'		:		menuAuth,
                    	'fpDate'		:		function(){return inWhsDateCal.replace(/-/g,"");},
    ...
    function monthProductPresentTableFunc2(){
        ...
        let tempTableData = $('#planManTable').on( 'error.dt', function ( e, settings, techNote, message ) {
        		...
        		ajax: {
                    url: '<c:url value="/wm/monthProductionManMTP"/>',
                    type: 'POST',
                    data: {
                    	'menuAuth'		:		menuAuth,
                    	'fpDate'		:		function(){return inWhsDateCal.replace(/-/g,"");},
	...
	$('#btnJobChange').on('click',function(){
		...
		$.ajax({
    		url: '<c:url value="wm/checkJobChangeMTP"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
	...
	$('#btnSaveJobChange').on('click', function() {
		...
		if(check){
			$.ajax({
	            url: '<c:url value="/wm/insertMonthProductionPlanJobChangeMTP"/>',
	            type: 'POST',
	            data: JSON.stringify(dataList),
	            dataType:"json",
	            contentType : "application/json; charset=UTF-8",
   	...
   	let jobChangeTable =$('#jobChangeTable').on( 'error.dt', function ( e, settings, techNote, message ) {
   				...
   				pageLength : 20,
   				ajax : {
   					url : '<c:url value="wm/jobChangeModalMTP"/>',
   					type : 'GET',
   					data : {
   	...
   	function selectGoodsCode()
   	{
		if(goodsCdPopUpTable == null || goodsCdPopUpTable == undefined)	{
			goodsCdPopUpTable = $('#goodsCdPopUpTable').DataTable({
		        ...
		        pageLength: 15,
		        ajax: {
		            url: '<c:url value="/wm/jobChangeGoodsModalMTP"/>',
		            type: 'GET',
		            data: {
		            	'menuAuth'	 	: 		menuAuth,
    ...
    $('#btnPlanSave').on('click', function() {
		...
		$.ajax({
            url: '<c:url value="/wm/createManMTP"/>',
            type: 'POST',
            data: JSON.stringify(dataList),
            dataType:"json",
            contentType : "application/json; charset=UTF-8",
    ...
    $('#btnYeild').on('click', function() {
    	$.ajax({
			url: '<c:url value="wm/monthProductionPlanYeildReadMTP"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
   ...
   //잡체인지 목록 테이블
  	let jcListTable =$('#jcListTable').on( 'error.dt', function ( e, settings, techNote, message ) {
			...
			pageLength : 20,
			ajax : {
				url : '<c:url value="wm/jcModalListMTP"/>',
				type : 'GET',
				data : {				
	...
	$('#jobChangeDelete').on('click', function() {
		...
		if(equipStat){
			...
			$.ajax({
	            url: '<c:url value="/wm/deleteMonthProductionPlanMTP"/>',
	            type: 'POST',
	            data: JSON.stringify(dataList),
	            dataType:"json",
	            contentType : "application/json; charset=UTF-8",
	...
	$('#btnDeleteConfirm').on('click', function() {
		$.ajax({
    		url: '<c:url value="wm/deleteMonthProductionPlanMainMTP"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
