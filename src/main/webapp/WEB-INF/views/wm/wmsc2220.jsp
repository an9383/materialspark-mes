		...
		<div class="row col-sm-6">
			<nav aria-label="breadcrumb" class="breadcrumb-padding">
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item active">월별 설비 JC 및 PM 일정(변경판)</li>
				</ol>
			</nav>
		</div>
...
<div class="modal fade" id="editCheckPopupModal" tabindex="-1" role="dialog" aria-labelledby="editCheckPopupModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="row">					
					<label class="input-label-sm">이미 확정된 계획입니다. 수정하시겠습니까? </label>
			...
			<div class="modal-body">
				...
				<div class="infoModal" style="height: 78vh; border-bottom: 1px solid #e3e3e3; overflow:scroll;" id="jcListDiv">
					<h5>1. 최초등록전 기준정보관리에 제조조건관리에서 각 <span style="color:red">A호기</span>들에 <span style="color:red">메인구분과 FP사용유무</span>를 설정한다.<br>
						...
						4. 입력버튼을 누르고 비가동구분과 시간을 입력한다.<br><br>
						
						5. 저장 버튼을 클릭한다.<br><br>
						
						6. 잡체인지 버튼을 눌러 잡체인지를 등록한다.<br><br>
						
						7. 잡체인지를 입력할때 완전비가동이 아닌 날짜를 잡체인지 일자에 넣고 잡체인지를 입력한다. <br>
		...
		<div class="modal-content">
			...
			<div class="modal-body">				
					...
					<table id="yeildTable" class="table table-bordered">
						...
						<tr>
							<td><input type="text" class="form-control text-right number-float3" style="min-width:100%;" id="yeild1" value="0" disabled></td>
							<td><input type="text" class="form-control text-right number-float3" style="min-width:100%;" id="yeild2" value="0" disabled></td>												
						</tr>
...
<script>
	...
	let menuAuth = 'wmsc2220';
	let currentHref = 'wmsc2220';
	let currentPage = $('.'+currentHref).attr('id');
	$('#'+currentPage).closest('.has-child','li').addClass('has-open has-active');
	$('#'+currentPage).closest('.menu-item').addClass('has-active'); 
	$(document).attr("title","월별 설비 JC 및 PM 일정(변경판)");	
    ...
    $('#btnEdit').on('click', function() {        	
    	...
    	$.ajax({
    		url: '<c:url value="wm/checkCompleteYnMTP2"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
				'fpDate'	:	function(){return inWhsDateCal.replace(/-/g,"");},
				'cplGubun'	:	function(){return "002";},				
			},
    ...
    $('#btnComplete').on('click', function() {  
    	...
    	$.ajax({
    		url: '<c:url value="wm/updateCompleteYnModifyMTP2"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
	...
	$('#btnSave').on('click', function() {
		...
		$.ajax({
            url: '<c:url value="/wm/insertMonthProductionPlanMTP2"/>',
            type: 'POST',
...
/* 		$.each(npGubunList,function(index,item){
			...
			$.ajax({
				...
				success: function(res){
					...
					if (res.result="ok"){
						...
						var planHeader = '<th  rowspan="2" class="text-center"style="vertical-align:middle; min-width:100px;">설비</th>';
							planHeader += '<th rowspan="2" class="text-center"style="vertical-align:middle; min-width:100px;">제품명</th>';
							planHeader += '<th rowspan="2" class="text-center"style="vertical-align:middle; min-width:100px;">재질</th>';				
						var planHeader2 = '';
						
						for (var i=0; i<data.length; i++) {
							//존재하지 않는 날짜
							if (data[i].day == 0) {
								planHeader2 += '<th class="text-center day' + (i+1) + '" style="vertical-align:middle; color:orange; min-width:70px;">' + data[i].countDay + '</th>';
								planHeader += '<th class="text-center"><input type="checkBox" class="checkBoxDate" id="checkDay' + (i+1) + '" name="checkBoxDate"> </th>';
							//토요일
							} else if (data[i].day == 7) {
								planHeader2 += '<th class="text-center day' + (i+1) + '" style="vertical-align:middle; color:blue; min-width:70px;">' + data[i].countDay + '</th>';
								planHeader += '<th class="text-center"><input type="checkBox" class="checkBoxDate" id="checkDay' + (i+1) + '" name="checkBoxDate"> </th>';
							//일요일
							} else if (data[i].day == 1){
								planHeader2 += '<th class="text-center day' + (i+1) + '" style="vertical-align:middle; color:red; min-width:70px;">' + data[i].countDay + '</th>';
								planHeader += '<th class="text-center"><input type="checkBox" class="checkBoxDate" id="checkDay' + (i+1) + '" name="checkBoxDate" > </th>';
							}
							//그외
							else {
								planHeader2 += '<th class="text-center day' + (i+1) + '" style="vertical-align:middle; min-width:70px;">' + data[i].countDay + '</th>';
								planHeader += '<th class="text-center"><input type="checkBox" class="checkBoxDate" id="checkDay' + (i+1) + '" name="checkBoxDate" > </th>';
							}
						}
				
						$('#planHeader').html(planHeader).promise().done(function(){
							$('#planHeader2').html(planHeader2);
    ...
    function monthProductPresentTableFunc(){
        ...
        let tempTableData = $('#productionPlanTable').on( 'error.dt', function ( e, settings, techNote, message ) {
        		...
        		ajax: {
                    url: '<c:url value="/wm/readMonthPlanBodySLDModifyMTP2"/>',
                    type: 'POST',
                    data: {
                    	'menuAuth'		:		menuAuth,
                    	'fpDate'		:		function(){return inWhsDateCal.replace(/-/g,"");},
        		...
        		drawCallback:function( settings){
        			...
        			$.each($('#productionPlanTable tbody tr'),function(index, item){
        				if($('.day27').text()==""){
    ...
    function monthProductPresentTableFunc2(){
        ...
        let tempTableData = $('#planManTable').on( 'error.dt', function ( e, settings, techNote, message ) {
        		...
        		ajax: {
                    url: '<c:url value="/wm/monthProductionManMTP2"/>',
                    type: 'POST',
                    data: {
                    	'menuAuth'		:		menuAuth,
                    	'fpDate'		:		function(){return inWhsDateCal.replace(/-/g,"");},
	...
	$('#btnJobChange').on('click',function(){
		...
		$.ajax({
    		url: '<c:url value="wm/checkJobChangeMTP2"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
	...
	$('#btnSaveJobChange').on('click', function() {
		...
		if(check){
			$.ajax({
	            url: '<c:url value="/wm/insertMonthProductionPlanJobChangeMTP2"/>',
	            type: 'POST',
	            data: JSON.stringify(dataList),
	            dataType:"json",
	            contentType : "application/json; charset=UTF-8",
   	...
   	let jobChangeTable =$('#jobChangeTable').on( 'error.dt', function ( e, settings, techNote, message ) {
   				...
   				pageLength : 20,
   				ajax : {
   					url : '<c:url value="wm/jobChangeModalMTP2"/>',
   					type : 'GET',
   					data : {
   	...
   	function selectGoodsCode()
   	{
		if(goodsCdPopUpTable == null || goodsCdPopUpTable == undefined)	{
			goodsCdPopUpTable = $('#goodsCdPopUpTable').DataTable({
		        ...
		        pageLength: 15,
		        ajax: {
		            url: '<c:url value="/wm/jobChangeGoodsModalMTP2"/>',
		            type: 'GET',
		            data: {
		            	'menuAuth'	 	: 		menuAuth,
    ...
    $('#btnPlanSave').on('click', function() {
		...
		$.ajax({
            url: '<c:url value="/wm/createManMTP2"/>',
            type: 'POST',
            data: JSON.stringify(dataList),
            dataType:"json",
            contentType : "application/json; charset=UTF-8",
    ...
    $('#btnYeild').on('click', function() {
    	$.ajax({
			url: '<c:url value="wm/monthProductionPlanYeildReadMTP2"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
   ...
   //잡체인지 목록 테이블
  	let jcListTable =$('#jcListTable').on( 'error.dt', function ( e, settings, techNote, message ) {
			...
			pageLength : 20,
			ajax : {
				url : '<c:url value="wm/jcModalListMTP2"/>',
				type : 'GET',
				data : {				
	...
	$('#jobChangeDelete').on('click', function() {
		...
		if(equipStat){
			...
			$.ajax({
	            url: '<c:url value="/wm/deleteMonthProductionPlanMTP2"/>',
	            type: 'POST',
	            data: JSON.stringify(dataList),
	            dataType:"json",
	            contentType : "application/json; charset=UTF-8",
	...
	$('#btnDeleteConfirm').on('click', function() {
		$.ajax({
    		url: '<c:url value="wm/deleteMonthProductionPlanMainMTP2"/>',
			type: 'POST',  
			async: false,
			data: {
				'menuAuth'	:	menuAuth,
