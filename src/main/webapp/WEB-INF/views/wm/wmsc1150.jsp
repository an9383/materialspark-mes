...
<div class="page-wrapper" id="page-wrapper">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="deleteAppearCheckModalLabel">재검등록 삭제 확인</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
	...
	<div class="modal fade" id="stateCheckPopupModal" tabindex="-1" role="dialog" aria-labelledby="stateChestateLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="stateCheckModalLabel">재검등록 상태 확인</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
	...
	<div class="modal fade" id="testDonePopupModal" tabindex="-1" role="dialog" aria-labelledby="testDonePopupModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="testDoneModalLabel">재검등록 검사상태 확인</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">재검등록</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
...
<!-- 					<button type="button" class="btn btn-primary float-right touch6" onclick="btnScrollBottom();" style="margin-bottom:5px;">▼</button> -->
				...
				<div id="inspDiv">
					<div class="col-md-12 matrl-bottom-list" style="min-height:325px; overflow:scroll;">
						<table class="table table-bordered" id="outputCardTable" style="padding-left:0px; padding-right:0px;">
							<thead class="thead-light">
							<tr>
								<th class="text-center" style="min-width:70px;">재검등록일</th>
								...
								<th class="text-center" style="min-width:65px;">출하재검<br>요청</th>
			...
			<div class="right-list right-sidebar" id="myrSidenav" style="width:34%; padding-top:0px; padding-bottom:0px;">
				...
				<div class="card-body col-sm-12" style="padding: 1rem 0px 1rem 0px;">
					<button type="button" class="btn btn-primary float-left touch6" id="btnSave" disabled>재검완료</button>
						...
						<button type="button" class="btn btn-primary float-left touch6 ml-5" onclick="btnScrollTop();">▲</button>
					...
					<!-- <button type="button" class="btn btn-warning float-left touch6 ml-1 d-none" id="btnDis" disabled>폐기</button> -->
					
					<!-- <label class='ml-4' style="color: cornflowerblue; font-size: 20px;font-weight: 300;text-decoration: underline solid 1px;" id="workOrdNoTitle"></label> -->
	...
	<!-- 								<td><select class="custom-select custom-select-lg touch6" id="edgeSampleCnt" disabled></select></td> -->
								...
								<tr>
									...
									<td colspan="3">
										<div style="display: flex;flex-wrap: nowrap;justify-content: center;">
											<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:65px;"disabled>1</button>
											<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:65px;"disabled>2</button>
											<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:65px;"disabled>3</button>
											<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:65px;"disabled>4</button>
											<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:65px;"disabled>5</button>
											<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:65px;"disabled>6</button>
								...
								<th class="touch6">부적합 유형</th>
									<td><select class="custom-select custom-select-lg touch6" id="faultyGubun" disabled>
											<option value="001">major</option>
											<option value="002">minor</option>
									</select></td>
									
									<th colspan="2"></th>
									<!-- 현재 솔브레인에서는 재검 등록 시 무조건 합격처리로 진행됨_2022.11.10 -->
									<th class="touch6 d-none">합불판정</th>
									<td class="d-none">
										<select class="custom-select custom-select-lg touch6" id="inspStatus" disabled>
								...
								<tr>	
									...
									<td><select class="custom-select custom-select-lg touch6" id="inspGubun" disabled>
											<option value="">미선택</option>
								...
								<tr>
									<!-- <th class="touch6">재검 유형</th>
									<td >
										<select class="custom-select custom-select-lg touch6" id="inspGubun" disabled>									
											<option value="001">부분재검</option>
											<option value="002">전체재검</option>
										</select>
										
									</td>  -->
									<th class="touch6">불량유형</th>
									<td colspan="3" id="beforeTotalBad" style="text-align: center">0</td>
								</tr>
	    ...
	    <div class="modal-dialog modal-lg" role="document" style="width: 85vh;">
	        <div class="modal-content" style="width: 60%; margin-left: 30%; margin-top: 20%;" id="workerChangeContent">
	            ...
	            <div class="modal-body">
	            	<br>
					<div class="mb-2 row">
						<label class="input-label-sm touch6 d-none">Metal Lot No</label>
						<input type="text" class="form-control touch6 d-none" id="lotAdd" name="lotAdd" value="" style="min-width:100%;">
...
<script>
	...
	let menuAuth = 'wmsc1150';
	let currentHref = 'wmsc1150';
	let currentPage = $('.'+currentHref).attr('id');
	$('#'+currentPage).closest('.has-child','li').addClass('has-open has-active');
	$('#'+currentPage).closest('.menu-item').addClass('has-active');
	$(document).attr("title","재검등록");
	...
	//공통코드 처리 시작
	var goodsFaultyBigCd=new Array();	//재검등록 대분류
	...
	var systemCommonCode=new Array();	//재검등록 소분류
    ...
    let outputCardTable = $('#outputCardTable').DataTable({
        ...
        ajax: {
			...
			data: {
				...
				'reInspStatus'			:	function(){return '002';},
...
/* 			// 작업지시 수정 버튼 click
			$('.btnReInsp').off('click',function() {});
			$('.btnReInsp').on('click',function() {
				let idx = $(this).data('idx');
				console.log(idx);
				let tr = $(this).parent().parent();
				let data = outputCardTable.row(tr).data();

				if(data.inspStatus == '001'){
					$.ajax({
						url: '<c:url value="qm/shipInspectReInspStatusUpdate2"/>',
			            type: 'POST',
			            data: {		      
				            'trayNo'			: 	idx,
				            'reInspStatus'		:   '002'
						...
						success : function(res) {
							if (res.result == "ok") { //응답결과
								toastr.success('출하재검으로 이관 되었습니다.');			
					...
					toastr.warning('불합격 처리된 트레이 입니다.');
					return false;
				}
			}); */
	...
	$('#outputCardTable tbody').on('click', 'tr', function () {
		...
		if ($(this).hasClass('selected')) {
			...
			$(this).addClass('selected');
			labelLotNoSeq = outputCardTable.row(this).data().labelLotNoSeq;	
			...
			//$('#workOrdNoTitle').text(outputCardTable.row(this).data().labelLotNoSeq);
			$('#my-spinner').show();
			
			$('.pushBtn').removeClass('btn-danger');
			$('.pushBtn').addClass('btn-primary');
			$('.pushBtn').attr('style', 'width:60px');
	...
	function scanWorkOrdOutputSubOrdLotNo() {
		let trayFaultSplit = '';
		$.ajax({
	    	url: '<c:url value="/qm/shipInspAdmReadSLDReInsp"/>',
	        type: 'GET',
	        data: {
	        	'menuAuth'	 	: 		menuAuth,
	        	'trayNo'		:		function() { return labelLotNoSeq; },
	        	'reInspStatus'	: 		'002',
	        	'factoryCode'	: 		'003',
		        'seq'			:		seq
	        },
	        success: function (res) {
	            ...
	            if (res.result == 'ok') {
	            	if (data[0].inspDate == null) {
						...
						$(".num_only").val("0");						
						$('#faultyCnt').val(addCommas(data[0].faultyCnt));$('#beforeTotalBad').text('');
						...
						//$('#reInspRange').val(data[0].reInspRange);
						$('#reInspSystem').val(data[0].reInspSystem);
						
						$('#beforeTotalBad').text(data[0].lastFaultyGubun);
						if(data[0].faultyTrayNo != null){
						...
						//여기
						$('#faultyGubun').val( $('#faultyGubun').val() == null ? "001" : $('#faultyGubun').val() );
						$('#reInspSystem').val( $('#reInspSystem').val() == '' ? "001" : $('#reInspSystem').val() );
						$('#inspGubun').val( $('#inspGubun').val() == '' ? "001" : $('#inspGubun').val() );
						
						if(res.listSeq == 'first'){ //출하검사에서 가져왔을경우.
							$('#btnEdit').attr('disabled',true);        
		                	$('#btnAdd').attr('disabled',false);     
		                	$('#btnDel').attr('disabled',true);     
		                	$('#btnAddLot').attr('disabled',true);		                    
	
						} else if(res.listSeq == 'second'){ //입력한것을 수정할 경우
							$('.num_only').attr('disabled', true);
							$('#btnSave').attr('disabled', true);
							$('#btnAdd').attr('disabled',true);     
							$('#btnEdit').attr('disabled',false);
							//삭제비활 $('#btnDel').attr('disabled',false);       
				...
				//출하불량 TB 상세조회
 		        $.ajax({
 		            ...
 		            data: {
 		            	...
 		            	'reInspStatus'	: 		'002',
 		            	'factoryCode'	: 		'003',
 		            	'trayNo'		:		function(){return labelLotNoSeq;},		//생산LorNo
 		            	'seq'			:		seq
	...
	$('#btnSave').on('click', function() {
       ...
       /*  if (parseInt(removeCommas($('#appearOutputCnt').val())) > 99999) {
        ...
        //if (trueFalse) {
			...
			if(faultyTotalCnt != 0 && faultyTrayNo == ""){
			...
			$.each($('#metalFaultyTbody .num_only'), function() {
				if($(this).val() != '0' && $(this).val() != '00') {
					...
					//rowData.reInspStatus  = '002';
					rowData.reInspStatus 	= outputCardTable.rows('.selected').data()[0].reInspStatus;;
					//rowData.reInspRange 	= $('#reInspRange').val();
    		...
    		url = '<c:url value="/qm/reShipInspectUpdate_fn3"/>';
...
/* 		if (equipCd == null || equipCd == '' || equipCd == ' ') {
	...
	$('#btnDeleteAppearCheck').on('click', function(){
		//삭제
		$.ajax({
			url : '<c:url value="qm/shipInspectDelete"/>',
			type : 'POST',
			data : {
				...
				'ordGubun'		:	function(){ return ""; },
				'packUnitNm'	:	function(){ return ""; },	
			},
			success : function(res) {
				let data = res.result;
				if (res.result == 'ok') {
...
// 		$('#badTotal6').text("0");
    ...
    function viewDisabled(flag){
    	...
    	//$('#reInspRange').attr('disabled',flag);
...
/* 			var sum1 = parseInt($("#metalMmb").val() || 0);
	...
	//			if( res.result == 'ok' ){
	//				if(searchData == true) {
	//					toastr.success("재검등록 불량 수정 되었습니다.");
	//				}
	//				else if(searchData == false){
	//					toastr.success("재검등록 불량 저장 되었습니다. 종물을 등록해 주세요!");
	//				}
...
//     		$('#workOrdNoScan').focus();
	...
	$('.pushBtn').on('click',function() {
		...
		if($(this).hasClass('btn-primary')) {
			...
			if( this.style.background != null && this.style.background != '' ) {
				let style_danger = 'width:60px;background: '+this.style.background+';border: 1px '+this.style.background+' solid;color: red;';
			...
			if( this.style.background != null && this.style.background != '' ) {
				let style_danger = 'width:60px;background: '+this.style.background+';border: 1px '+this.style.background+' solid;';
				$(this).attr('style', style_danger);
			}
		}
	...
	//Lot추가 테이블
	let lotAddTable = $('#lotAddTable').DataTable({
		language: lang_kor,
		lengthChange : false,
		paging: true,
		info: true,
		...
		pageLength: 20,
        ajax: {
            url: '<c:url value="/qm/getReTestAddLotNoReadt_F3"/>',
            type: 'GET',
            data: {
            	'labelLotNo'	: 		function(){return labelLotNoSeq;},
            },	              
        },
        rowId: 'workOrdNo',
        columns: [
	...
	$('#lotAddTable tbody').on('click', 'tr', function () {
   		if ($(this).hasClass('selected')) {
			...
			$('#lotAdd').val("");
			$('#lotAdd').val(lotAddTable.row(this).data().matrlLotNo);
		}
   	});
	
	//Lot추가 버튼 클릭 시
	function selectLotAddTable() {
		$('#lotAddTable').DataTable().ajax.reload(function(){
			$('#lotAddTable tbody tr').eq(0).click();				
		});
     	$('#lotAddModal').modal('show');   
	}
		...
		$.ajax({
			...
			success: function (res) {
				...
				if( res.result == 'ok' ) {
					...
					let style_MV	= 'width:60px;background: #ffd200;border: 1px #ffd200 solid;';
