...
<script>
	...
	let bizOrderTable = $('#bizOrderTable').DataTable({
        ...
        ajax : {
			url : '<c:url value="/mt/bizOrdWorkPrcssCurrentSituation"/>',
			type : 'POST',
			data : {
			...
			{ data: 'workStartTime', className : 'text-center align-middle',//작업시작시간
				render: function(data) {
					if(data != null && data != ''){
						...
						return '<div style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">'+moment(data,'YYYYMMDD HHmm').format('YYYY-MM-DD HH:mm')+'</div>';
					} else {
						return '';
					}
				}
