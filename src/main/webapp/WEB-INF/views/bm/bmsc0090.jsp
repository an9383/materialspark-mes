		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="layerPopUpLabel">창고정보/Location 관리</h5>
	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">창고정보/Location 관리</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
			...
			<div class="left-list left-30" id="left-30" style="width: 28%;">
				<div class="card">
					...
					<div class="table-responsive">
						<table class="table table-bordered" id="systemCommonGroupCd">
...
<script>
	...
	let menuAuth = 'bmsc0090';
	let currentHref = 'bmsc0090';
	let currentPage = $('.'+currentHref).attr('id');
	$('#'+currentPage).closest('.has-child','li').addClass('has-open has-active');
	$('#'+currentPage).closest('.menu-item').addClass('has-active');  
	$(document).attr("title","창고관리/Location");
	...
	let systemCommonGroupCd = $('#systemCommonGroupCd').on( 'error.dt', function ( e, settings, techNote, message ) {
	    	if(techNote == 7) {
        ...
        pageLength: 20,
		ajax: {
			url: '<c:url value="bm/locationGroupDataList"/>',
			type: 'GET',
			data: {
				'menuAuth'	 :	menuAuth,
			},
	...
	$('#systemCommonGroupCd_length').html(htmlGroup);
	...
	let systemCommonCd = $('#systemCommonCd').on( 'error.dt', function ( e, settings, techNote, message ) {
        ...
        pageLength: 20,	
		'ajax': {
				url: '<c:url value="bm/locationCodeDataList"/>',
				type: 'GET',
				data: {
					'menuAuth'	 : menuAuth,
	...
	$('#systemCommonCd_length').html(htmlCd);
   	...
   	$('#baseGroupAdd').click(function () {
   		...
   		//기초그룹코드 조회
   		$.ajax({
        	url: '<c:url value="bm/readLocationGroupSeq"/>',
            type: 'GET',
            data: {
            	'menuAuth'	 : menuAuth,
			},
   	...
   	$('#baseGroupEdit').click(function () {
   		var data = systemCommonGroupCd.row('.selected').data();
   	...
   	$('#baseCdAdd').click(function () {
   		...
   		if(data == null || data == undefined ) {
	   		...
	   		//기초코드상세 코드 조회
	   		$.ajax({
	        	url: '<c:url value="bm/readLocationCommonSeq"/>',
	            type: 'GET',
	            data: {
	            	'menuAuth'	 : menuAuth,
   	...
   	$('#baseCdEdit').click(function () {
   		var dataGrp = systemCommonGroupCd.row('.selected').data();
	...
	// 시스템공통그룹 상세 정보 보기
	$('#systemCommonGroupCd tbody').on('click', 'tr', function () {

        if ( $(this).hasClass('selected') ) {
            $(this).removeClass('selected');
        }
        else {
        	$('#systemCommonGroupCd').DataTable().$('tr.selected').removeClass('selected');
            $(this).addClass('selected');
            
	        baseGroupCd = systemCommonGroupCd.row( this ).data().baseGroupCd;
	...
	function baseGroupCdProc(puFlag)
	{
	    ...
	    if(puFlag == "groupCdAdd") {
	    	groupUrl = '<c:url value="bm/locationGroupCreate"/>';
	    	groupMsg = '기초코드가 등록 되었습니다.';
	    } else {
	    	groupUrl = '<c:url value="bm/locationGroupUpdate"/>';
	    ...
	    $.ajax({
	        ...
	        success: function (res) {
	            ...
	            if (res.result == 'ok') {
		            ...
		            $('#systemCommonGroupCd').DataTable().ajax.reload(); //등록 후 테이블 리로드	            	
	            	$('#layerPopUpModal').modal('hide');
	...
	function baseCdProc(puFlag)
	{
	    ...
	    var cdMsg;
	    if(puFlag == "cdAdd") {
	    	cdUrl = '<c:url value="bm/locationCodeCreate"/>';
	    	cdMsg = '세부코드가 등록 되었습니다.';
	    } else {
	    	cdUrl = '<c:url value="bm/locationCodeUpdate"/>';
