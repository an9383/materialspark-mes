		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="layerPopUpLabel">작업장 정보관리</h5>
	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">작업장 정보관리</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
...
<script>
	...
	let systemCommonGroupCd = $('#systemCommonGroupCd').on( 'error.dt', function ( e, settings, techNote, message ) {
        ...
        pageLength: 20,
		ajax: {
			url: '<c:url value="bm/workPlaceGroupDataList"/>',
			type: 'GET',
			data: {
				'menuAuth'	 :	menuAuth,
			},
	...
	let systemCommonCd = $('#systemCommonCd').DataTable({
        ...
        pageLength: 20,	
		'ajax': {
				url: '<c:url value="bm/workPlaceCodeDataList"/>',
				type: 'GET',
				data: {
					'menuAuth'	 : menuAuth,
	...
	$('#systemCommonCd_length').html(htmlCd);
   	...
   	$('#baseGroupAdd').click(function () {
   		...
   		//기초그룹코드 조회
   		$.ajax({
        	url: '<c:url value="bm/readWorkPlaceGroupSeq"/>',
            type: 'GET',
            data: {
            	'menuAuth'	 : menuAuth,
			},
   	...
   	$('#baseCdAdd').click(function () {
   		...
   		if(data == null || data == undefined ) {
	   		...
	   		//기초코드상세 코드 조회
	   		$.ajax({
	        	url: '<c:url value="bm/readWorkPlaceSeq"/>',
	            type: 'GET',
	            data: {
	            	'menuAuth'	 : menuAuth,
	...
	function baseGroupCdProc(puFlag)
	{
	    ...
	    if(puFlag == "groupCdAdd") {
	    	groupUrl = '<c:url value="bm/workPlaceGroupCreate"/>';
	    	groupMsg = '기초코드가 등록 되었습니다.';
	    } else {
	    	groupUrl = '<c:url value="bm/workPlaceGroupUpdate"/>';
	...
	function baseCdProc(puFlag)
	{
	    ...
	    var cdMsg;
	    if(puFlag == "cdAdd") {
	    	cdUrl = '<c:url value="bm/workPlaceCodeCreate"/>';
	    	cdMsg = '세부코드가 등록 되었습니다.';
	    } else {
	    	cdUrl = '<c:url value="bm/workPlaceCodeUpdate"/>';
