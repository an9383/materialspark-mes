	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">시스템공통코드관리</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
...
<script>
	...
	let systemCommonGroupCd = $('#systemCommonGroupCd').on( 'error.dt', function ( e, settings, techNote, message ) {
        ...
        pageLength: 20,
		ajax: {
			url: '<c:url value="bm/systemCommonGroupDataList"/>',
			type: 'GET',
			data: {
				'menuAuth'	 :	menuAuth,
			},
	...
	$('#systemCommonCd_length').html(htmlCd);
   	...
   	$('#baseGroupAdd').click(function () {
   		...
   		//기초그룹코드 조회
   		$.ajax({
        	url: '<c:url value="bm/readGroupSeq"/>',
            type: 'GET',
            data: {
            	'menuAuth'	 : menuAuth,
			},
	...
	function baseGroupCdProc(puFlag)
	{
	    ...
	    if(puFlag == "groupCdAdd") {
	    	groupUrl = '<c:url value="bm/systemCommonGroupCreate"/>';
	    	groupMsg = '기초코드가 등록 되었습니다.';
	    } else {
	    	groupUrl = '<c:url value="bm/systemCommonGroupUpdate"/>';
