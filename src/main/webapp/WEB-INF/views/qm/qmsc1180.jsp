...
<div class="page-wrapper" id="page-wrapper">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="deleteAppearCheckModalLabel">출하검사 삭제 확인</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
	...
	<!-- 출하검사 폐기 확인모달 시작-->
	<div class="modal fade" id="disAppearCheckPopupModal" tabindex="-1" role="dialog" aria-labelledby="disAppearCheckPopupModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="disAppearCheckPopupModalLabel">출하검사 폐기 확인</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
					<h6>폐기하시겠습니까?</h6>
					<p>*마지막에 등록한 항목만 폐기가 가능합니다!</p>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-danger touch6" id="btnDisAppearCheck" data-dismiss="modal" style="min-width: 70px;">폐기</button>
					<button type="button" class="btn btn btn-secondary touch6" id="" data-dismiss="modal" style="min-width: 70px;">닫기</button>
				</div>
			</div>
		</div>
	...
	<div class="modal fade" id="stateCheckPopupModal" tabindex="-1" role="dialog" aria-labelledby="stateChestateLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="stateCheckModalLabel">출하검사 상태 확인</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
	...
	<div class="modal fade" id="testDonePopupModal" tabindex="-1" role="dialog" aria-labelledby="testDonePopupModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="testDoneModalLabel">출하검사 검사상태 확인</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">출하검사</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
	<div class="container-fluid" id="main" style="padding-bottom:0px; padding-right:0px;">
		<div class="row table-wrap-hid" id="mainCard">			
			<div class="right-list workOrder-left-list" id="middleList" style="width:65%; margin-right:3px;">
				<!-- 작업지시 정보 시작-->
				<div class="card-body" style="padding-top:5px; padding-bottom:5px;">
					<div class="row" style="position:relative">
						&nbsp;
						<div class="row p-1 d-none" id="ordDateDiv" >
						...
						<div class="row mt-1">
							<label class="input-label-sm touch6">트레이 라벨 스캔</label>
						...
						<button type="button" class="btn btn-primary float-right mr-5 touch6" id="btnWorkOrdRetv" style="margin-top:3px;">전체조회</button>
						
						<div class="row">
							<label>M(메탈)교체 : </label>
							<div style="background-color: purple;width: 20px;height: 20px;border-radius: 50%;margin-left: 3px;"></div>
						</div>
						
						<div class="row ml-3">
							<label>Vision Tray : </label>
							<div style="background-color: #f8cbad;width: 20px;height: 20px;border-radius: 50%;margin-left: 3px;"></div>
						</div>
						
						<div class="row ml-3">
							<label>M(메탈)교체+Vision Tray : </label>
							<div style="background-color: #ffd200;width: 20px;height: 20px;border-radius: 50%;margin-left: 3px;"></div>
						</div>
						
						<div class="row ml-4">
							<button type="button" class="btn btn-primary float-left mr-2 touch6" id="btnReInspCancel">재검요청취소</button>
						</div>
				...
				<div class="card d-none" id="workOrderTableCard">
<!-- 					<button type="button" class="btn btn-primary float-right touch6" onclick="btnScrollTop();" style="margin-bottom:5px;">▲</button> -->
					<div class="col-md-12 matrl-top-list" id="workOrderScrollDiv" style="overflow:scroll; min-height:350px; max-height:350px;">
...
<!-- 					<button type="button" class="btn btn-primary float-right touch6" onclick="btnScrollBottom();" style="margin-bottom:5px;">▼</button> -->
				...
				<div class="card-header card-tab">
					<ul class="nav nav-tabs card-header-tabs">
						<li class="nav-item"><a class="nav-link active show" data-toggle="tab" id="inspTab1">출하검사</a></li>
						<li class="nav-item"><a class="nav-link" data-toggle="tab" id="inspTab2">출하재검사</a></li>							
					</ul>
				</div>
				<!-- 작업지시 정보 끝 -->
	<!-- 			<div class="card-body col-sm-12" style="padding: 1px;">
				...
				<div id="inspDiv">
					<div class="col-md-12 matrl-bottom-list" style="min-height:325px; overflow:scroll;">
						<table class="table table-bordered" id="outputCardTable" style="padding-left:0px; padding-right:0px;">
							<thead class="thead-light">
							<tr>
								<th class="text-center" style="min-width:70px;">출하 검사일</th>
								<th class="text-center" style="min-width:80px;">주재료</th>
								<th class="text-center" style="min-width:100px;">트레이번호(6)</th>
								<th class="text-center" style="min-width:60px;">출하<br>양품수량</th>
								<th class="text-center" style="min-width:50px;">총<br>불량수량</th>
								<th class="text-center" style="min-width:40px;">총<br>불량률</th>
								<th class="text-center" style="min-width:60px;">불량TRAY</th>
								...
								<th class="text-center" style="min-width:60px;">차수</th>
								<th class="text-center" style="min-width:60px;">재검<br>요청</th>
							</tr>
							</thead>
							<tfoot>
								<tr>
									...
									<td>총 합계</td>
									<td id="outputCntSum" class="text-right" colspan="5">0</td>
								</tr>
							</tfoot>
						</table>
					</div>
					<div class="col-md-12 matrl-bottom-list" style="min-height:325px; overflow:scroll;">
						<table class="table table-bordered" id="outputCardTableRecord" style="padding-left:0px; padding-right:0px;">
							<thead class="thead-light">
							<tr>
								<th class="text-center" style="min-width:70px;">검사일</th>
								<th class="text-center" style="min-width:70px;">합불시간</th>
								<th class="text-center" style="min-width:100px;">트레이번호</th>
								<th class="text-center" style="width:30px;">차수</th>
								<th class="text-center" style="min-width:60px;">검사담당</th>
								<th class="text-center" style="min-width:80px;">검사구분</th>
								<th class="text-center" style="min-width:80px;">합불판정</th>
								<th class="text-center" style="min-width:60px;">불량TRAY</th>	
								<th class="text-center" style="min-width:60px;">양품수량</th>
								<th class="text-center" style="min-width:60px;">총불량수량</th>							
								<th class="text-center" style="min-width:90px;">불량유형</th>
				...
				<div id="reInspDiv" class="d-none">
					<div class="col-md-12 matrl-bottom-list" style="min-height:325px; overflow:scroll;">
						<table class="table table-bordered" id="outputCardTable2" style="padding-left:0px; padding-right:0px;">
							<thead class="thead-light">
							<tr>
								<th class="text-center" style="min-width:70px;">출하 검사일</th>
								<th class="text-center" style="min-width:80px;">주재료</th>
								<th class="text-center" style="min-width:100px;">트레이번호(3)</th>
								<th class="text-center" style="min-width:60px;">출하<br>양품수량</th>
								<th class="text-center" style="min-width:50px;">총<br>불량수량</th>
								<th class="text-center" style="min-width:40px;">총<br>불량률</th>
								<th class="text-center" style="min-width:60px;">불량TRAY</th>
								...
								<th class="text-center" style="min-width:40px;">상태</th>
								<th class="text-center" style="min-width:65px;">생산<br>작업자</th>
								<th class="text-center" style="min-width:65px;">차수</th>
								<th class="text-center" style="min-width:60px;">재검<br>요청</th>
							...
							<tfoot>
								<tr>
									<td colspan="3" style="text-align: center">합계</td>
									<td id="appearOutputSum2" style="text-align:right">0</td>
									<td id="appearFaultySum2" style="text-align:right">0</td>
									<td id="appearTotalRatio2" style="text-align:right">0</td>
									<td>총 합계</td>
									<td id="outputCntSum2" class="text-right" colspan="5">0</td>
								</tr>
							</tfoot>
						</table>
					</div>
					<div class="col-md-12 matrl-bottom-list" style="min-height:325px; overflow:scroll;">
						<table class="table table-bordered" id="outputCardTableRecord2" style="padding-left:0px; padding-right:0px;">
							<thead class="thead-light">
							<tr>
								<th class="text-center" style="min-width:70px;">검사일</th>
								<th class="text-center" style="min-width:100px;">트레이번호</th>
			...
			<!--======================== .right-sidebar 등록,수정 ========================-->
			<div class="right-list right-sidebar" id="myrSidenav" style="width:34%; padding-top:0px; padding-bottom:0px;">
				
				<div class="card-body col-sm-12"  style="padding: 1rem 0px 1rem 0px;">
					<button type="button" class="btn btn-primary float-left mr-2 touch6" id="btnReInsp" disabled>재검요청</button>
					<button type="button" class="btn btn-primary float-left mr-2 touch6" id="btnSave" disabled>저장</button>
					
					<button type="button" class="btn btn-primary float-left touch6" id="btnInputCompletion">입력완료</button>
					...
					<button type="button" class="btn btn-primary float-left touch6 ml-3" onclick="btnScrollTop();">▲</button>
					<button type="button" class="btn btn-primary float-left touch6 ml-2" onclick="btnScrollBottom();">▼</button>
					...
					<button type="button" class="btn btn-danger float-left touch6 ml-1 d-none" id="btnDel" disabled>삭제</button>
					<!-- <button type="button" class="btn btn-warning float-left touch6 ml-1" id="btnDis" disabled>폐기</button>
					 -->
					...
					<button type="button" class="btn btn-primary float-right touch6" style="margin-right:5px;" id="btnAddLot" disabled>LOT추가</button>
					<!-- <button type="button" class="btn btn-success float-right touch6" style="margin-right:5px;" id="btnPrint" disabled>인쇄(엑셀 다운)</button> -->
				</div>
				
				<div style="height: 60%; overflow-y: auto; margin-bottom: 3%; border: 1px solid #dee2e6;" id="divScrollMove" >
					<div class="card" id="formBox"> <!-- card -->
						<div class="row mb-2">
							<!-- 작지 스캔 테이블 -->
							<table class="table table-bordered" id="workOrdScanTable">
								<colgroup>
									<col width="25%">
									<col width="25%">
									<col width="25%">
								...
								<tr>							                            
		                            <th class="touch6">출하 검사일</th>
		                            <td><input type="date" class="form-control touch6" id="inspDate" name="inspDate" disabled></td>
								...
								<tr>
									<th class="touch6">생산일자</th>
		                            <td><input type="date" class="form-control touch6" id="workOrdDate"	name="workOrdDate" disabled></td>
		                            
									<th class="touch6">생산 주/야</th>
									<td><select class="custom-select custom-select-lg touch6" id="dayNightGubun" style="height:30px;" disabled></select></td>
	...
	<!-- 								<td><select class="custom-select custom-select-lg touch6" id="edgeSampleCnt" disabled></select></td> -->
								...
								<!-- 	<th class="touch6">출하 양품수량</th>
									...
									<th class="touch6">총 출하불량수량</th>
									<td><input type="text" class="form-control text-right touch6" style="color:red;" id="faultyCnt" disabled></td>
									<th class="touch6">총 수량</th>
									<td><input type="text" class="form-control text-right touch6" style="color:red;" id="productQty" disabled></td>
									
								</tr>
								<tr>
									<th class="touch6">부적합 TRAY</th>
									<td colspan="3">
										<div style="display: flex;flex-wrap: nowrap;justify-content: center;">
											<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:65px; white-space: nowrap;"disabled>1</button>
											<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:65px; white-space: nowrap;"disabled>2</button>
											<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:65px; white-space: nowrap;"disabled>3</button>
											<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:65px; white-space: nowrap;"disabled>4</button>
											<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:65px; white-space: nowrap;"disabled>5</button>
											<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:65px; white-space: nowrap;"disabled>6</button>
											<input type="hidden"  id="tray1" >
											<input type="hidden"  id="tray2" >
											<input type="hidden"  id="tray3" >
											<input type="hidden"  id="tray4" >
											<input type="hidden"  id="tray5" >
											<input type="hidden"  id="tray6" >
										</div>
									</td>
								</tr>
								<tr>
								<th class="touch6">부적합 유형</th>
									<td><select class="custom-select custom-select-lg touch6" id="faultyGubun" disabled>
											<option value="">미선택</option>										
											<option value="001" id="faultyMajor">major</option>
											<option value="002" id="faultyMinor">minor</option>
									</select></td>
									<th class="touch6">합불판정</th>
									<td><select class="custom-select custom-select-lg touch6" id="inspStatus" disabled>
											<option value="">미선택</option>
								...
								<!-- </tr> -->
								<tr>	
									<th class="touch6">검사방법</th>
									<td><select class="custom-select custom-select-lg touch6" id="reInspSystem" disabled>
											<option value="">미선택</option>
											<option value="001">재검사</option>
											<option value="002">재작업</option>
									</select></td>			
									<th class="touch6">재검유형</th>
									<td>
										<select class="custom-select custom-select-lg touch6" id="inspGubun" disabled>
											<option value="">미선택</option>
											<option value="001">출하재검</option>
											<option value="002">부분재검</option>
											<option value="003">전수재검</option>
										</select>
									</td>							
								</tr>	
								<tr>
									<th class="touch6">비고</th>
									<td colspan="3"><input type="text" id="inspDesc" class="form-control touch6" value="" style="min-width:100%;" disabled ></td>
								</tr>
								<tr>	
									<th class="touch6">샘플수량</th>
									<td><input type="text" id="sampleCnt" class="form-control touch6" value="" style="min-width:100%;" disabled ></td>
									
									<th class="touch6">3Tray 중량</th>
									<td><input type="number" class="form-control touch6 num_only2" id="threeTrayWeight" name="threeTrayWeight" value="" style="min-width:100%;" max="100000" disabled></td>
								</tr>
								<tr>	
									<th class="touch6">3Tray 날짜</th>
									<td><select class="custom-select custom-select-lg touch6" id="trayWeightDate" disabled></select></td>
							...
							<div class="row col-sm-12">
								<div class="card-header card-tab col-md-10">
									<ul class="nav nav-tabs card-header-tabs">
	...
	<!-- 									<li class="nav-item"><a class="nav-link" data-toggle="tab" id="tab6">...</a></li> -->
								...
								<!-- <button type="button" class="btn btn-primary touch6" id="btnInputCompletion">입력완료</button> -->
							</div>
							<br>
						...
						<div class="card-body p-0 d-none">
							<label class="input-label-sm touch6">포장 구분 : </label>
							<select class="custom-select custom-select-md touch6" style="width:100px;" id="boxOrdGubunCd"></select>						
						</div>
						<br>
					</div><!-- card -->
				</div>
...
<!--LOT 추가 모달 -->
<div class="modal fade bs-example-modal-lg shadow-lg" id="lotAddModal" tabindex="-1" role="dialog" aria-labelledby="lotAddModalLabel" aria-hidden="true" data-keyboard="false">
	<div class="row">
	    <div class="modal-dialog modal-lg" role="document" style="width: 85vh;">
	        <div class="modal-content" style="width: 60%; margin-left: 30%; margin-top: 20%;" id="workerChangeContent">
	            <div class="modal-header">
	           		<h5 class="modal-title" id="lotAddModalLabel">LOT 추가</h5>	       
	            </div>
	            <div class="modal-body">
	            	<br>
					<div class="mb-2 row">
						<label class="input-label-sm touch6">Metal Lot No</label>
						<input type="text" class="form-control touch6" id="lotAdd" name="lotAdd" value="" style="min-width:100%;">	              
			        </div>			        
			    </div>
	            <div class="modal-footer" id="">
	            	<button type="button" class="btn btn btn-primary touch6" style="width: 16%;" id="btnModalLotAdd">저장</button>
...
<!-- 출하검사 재검요청 확인모달 시작-->
<div class="modal fade" id="ReInspConfirmPopupModal" tabindex="-1" role="dialog" aria-labelledby="ReInspConfirmPopupModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="ReInspConfirmPopupModalLabel">재검요청</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-primary touch6" id="btnReInspConfirm" data-dismiss="modal" style="min-width: 70px;">재검요청</button>
				<button type="button" class="btn btn btn-secondary touch6" id="" data-dismiss="modal" style="min-width: 70px;">닫기</button>
			</div>
		</div>
	</div>
...
<script>
	...
	let menuAuth = 'qmsc1180';
	let currentHref = 'qmsc1180';
	let currentPage = $('.'+currentHref).attr('id');
	$('#'+currentPage).closest('.has-child','li').addClass('has-open has-active');
	$('#'+currentPage).closest('.menu-item').addClass('has-active');
	$(document).attr("title","출하검사");
	var activeTab = '001';
	var sum = 0;
	...
	//공통코드 처리 시작
	var goodsFaultyBigCd=new Array();	//출하검사 대분류
	...
	var systemCommonCode=new Array();	//출하검사 소분류
	...
	var boxOrdGubunCode = new Array(); // 오더구분
	<c:forEach items="${boxOrdGubunCd}" var="info">
		var json=new Object();
		json.baseCd="${info.baseCd}";
		json.baseCdNm="${info.baseCdNm}";
		boxOrdGubunCode.push(json);
	</c:forEach> 
    //공통코드 처리 종료
...
// 	$('#badTotalNmae6').text(goodsFaultyBigCd[5].baseCdNm);
	...
	let productDateValue = '';
	$('#btnSave').attr('disabled', true);
	$('#btnedit').attr('disabled', true);
	$('.num_only').attr('disabled', true);
	$(".firstStep, .secondStep, .thridStep").attr("disabled",true);
	selectBoxAppend(ordGubunCode, "dayNightGubun", "" , "");
	selectBoxAppend(boxOrdGubunCode, "boxOrdGubunCd", "" , "");
...
// 	faultyHdCreate6();
	...
	$('#equipCodeAdmTable tbody').on('click', 'tr', function () {
		if ($(this).hasClass('selected')) {
			...
			$('#outputCardTable2').DataTable().clear().draw();
			$("#btnEdit").attr("disabled", true);
			$("#btnDel").attr("disabled", true);
			$("#btnDis").attr("disabled", true);
			$("#btnReInsp").attr("disabled", true);
			$("#btnSave").attr("disabled", true);
			...
			viewDisabled(true);
			viewValueClear();
		    $('.pushBtn').removeClass('btn-danger');
	    	$('.pushBtn').removeClass('btn-primary');
	    	$('.pushBtn').addClass('btn-primary');
		}
    });
    ...
    $('#btnWorkOrdRetv').on('click', function() {
		...
		$('#outputCardTable2').DataTable().ajax.reload( function () {});
    });
    ...
    let outputCardTable = $('#outputCardTable').DataTable({
		...
		pageLength: 10,
        ajax: {
			url: '<c:url value="/qm/shipInspSelectListFn3"/>',
			type: 'GET',
			async: false,
			data: {
				'menuAuth'			: 	menuAuth,
				'trayNo'			:	function(){return workOrdNoScan;},
				'reInspStatus'			:	function(){return '001';},
        		'searchGubun'			:	function(){return searchGubun;}
		...
		columns: [			
			...
			{
				render: function(data, type, row) {
					if((row['faultyCnt'] != null && data != 0) && (row['productQty'] != null && data != 0)) {
						let faultyPer = (row['faultyCnt'] / row['productQty']) * 100;
						return faultyPer.toFixed(2)+'%';
					} else {
						return "-";
					}
				}
...
/* 			{ className : 'text-center p-0',
				render : function(data, type, row, meta) {
					let html = '';
					if(row['inspDate'] != null) {
						html += '<button type="button" class="btn btn-primary mr-1 btnAddLotNo" id="btnAddLotNo-'+row['labelLotNo']+'" data-idx="'+row['labelLotNo']+'">추가</button>';
					} 				
					return html;
				}
			}, */
	    ...
	    columnDefs: [
	    	{ targets: [0,1,2,3,7,8,9,10,11], className: 'text-center' },
		...
		drawCallback: function () {
			var appearOutputSum 	= $('#outputCardTable').DataTable().column(3, {page: 'all'} ).data().sum();
			var appearFaultySum 	= $('#outputCardTable').DataTable().column(4, {page: 'all'} ).data().sum();
			var appearTotalRatio	= $('#outputCardTable').DataTable().column(5, {page: 'all'} ).data().sum();	//불량률
...
// 			var appearTotalRatio = 	0;
			...
			// 작업지시 수정 버튼 click
			$('.btnAddLotNo').off('click',function() {});
			$('.btnAddLotNo').on('click',function() {
				let idx = $(this).data('idx');

				$.ajax({
					url: '<c:url value="qm/getAddLotNo"/>',
		            type: 'POST',
		            data: {		      
			            'trayNo'			: 	idx,
		            	'equipCd'			:	equipCd,
					...
					success : function(res) {
						if (res.result == "ok") { //응답결과
		    				...
		    				let rowsOut 	= outputCardTable.rows('.selected').indexes();			       		
			       			let indexOut	= $('#outputCardTable').DataTable().row(rowsOut).index();		       		
				       		
			       			$('#outputCardTable').DataTable().ajax.reload(function(){
			       				$('#outputCardTable tbody tr').eq(indexOut).addClass('selected');
				       		});				
			       			$('#outputCardTableRecord').DataTable().ajax.reload( function () {});
	...
	//이력테이블
    let outputCardTableRecord = $('#outputCardTableRecord').DataTable({
		language: lang_kor,
		paging: false,
		info: false,
		ordering: true,
		...
		pageLength: 10,
        ajax: {
			url: '<c:url value="/qm/shipListRecord"/>',
			type: 'GET',
			async: false,
			data: {
				'menuAuth'			: 	menuAuth,
	    ...
	    columnDefs: [
	    	{ targets: [0,1,2,3,4,5,6,7,10], className: 'text-center' },
			{ targets: [8,9], render: $.fn.dataTable.render.number( ',' ) },
			{ targets: [8,9], className: 'text-right sum' }		
		],
		order: [
			[ 0, 'asc' ],
			[ 2, 'asc' ],
	...
	//생산실적상세 테이블
    let outputCardTable2 = $('#outputCardTable2').DataTable({
		language: lang_kor,
		paging: false,
		info: false,
		ordering: false,
        ...
        ajax: {
			...
			data: {
				'menuAuth'			: 	menuAuth,
				'trayNo'			:	function() {return workOrdNoScan;},
				'reInspStatus'			:	function(){return '003';},
				'searchGubun'			:	function(){return searchGubun;}
...
/* 			{ className : 'text-center p-0',
				render : function(data, type, row, meta) {
					let html = '';
					if(row['inspDate'] != null) {
						html += '<button type="button" class="btn btn-primary mr-1 btnAddLotNo" id="btnAddLotNo-'+row['labelLotNoSeq']+'" data-idx="'+row['labelLotNoSeq']+'">추가</button>';
					} 				
					return html;
				}
			}, */
		...
		drawCallback: function () {
			var appearOutputSum 	= $('#outputCardTable2').DataTable().column(3, {page: 'all'} ).data().sum();
			var appearFaultySum 	= $('#outputCardTable2').DataTable().column(4, {page: 'all'} ).data().sum();
			var appearTotalRatio	= $('#outputCardTable2').DataTable().column(5, {page: 'all'} ).data().sum();	//불량률
			var outputCntSum 		= parseInt(appearOutputSum) + parseInt(appearFaultySum);
// 			var appearTotalRatio = 	0;
			...
			$('#appearOutputSum2').text(addCommas(appearOutputSum));
			$('#appearFaultySum2').text(addCommas(appearFaultySum));
			$('#appearTotalRatio2').text((appearTotalRatio.toFixed(2))+'%');
			$('#outputCntSum2').text(addCommas(outputCntSum));
				...
				$.ajax({
					...
					success : function(res) {
						if (res.result == "ok") { //응답결과
		    				...
		    				let rowsOut 	= outputCardTable.rows('.selected').indexes();			       		
			       			let indexOut	= $('#outputCardTable2').DataTable().row(rowsOut).index();		       		
				       		
			       			$('#outputCardTable2').DataTable().ajax.reload(function(){
			       				$('#outputCardTable2 tbody tr').eq(indexOut).addClass('selected');
				       		});				
			       			$('#outputCardTableRecord2').DataTable().ajax.reload( function () {});
  	...
  	//이력테이블2
    let outputCardTableRecord2 = $('#outputCardTableRecord2').DataTable({
		language: lang_kor,
		paging: false,
		info: false,
		ordering: true,
		...
		pageLength: 10,
        ajax: {
			//url: '<c:url value="/qm/shipListRecordDtl"/>',
			url: '<c:url value="/qm/shipListRecord"/>',
			type: 'GET',
			async: false,
			data: {
	    ...
	    columnDefs: [
	    	{ targets: [0,1,2,3,4,5,6,9], className: 'text-center' },
			{ targets: [7,8], render: $.fn.dataTable.render.number( ',' ) },
			{ targets: [7,8], className: 'text-right sum' }		
		],
		order: [
			[ 0, 'asc' ],
			[ 1, 'asc' ],
	...
	$('#outputCardTable tbody').on('click', 'tr', function () {
		...
		if ($(this).hasClass('selected')) {
			...
			labelLotNo	= outputCardTable.row(this).data().labelLotNo;	
			shipInspIdx	= outputCardTable.row(this).data().idx;
			
			$('#productQty').val(outputCardTable.row(this).data().productQty);
			
			filmLotNo	= outputCardTable.row(this).data().filmLotNo;
			packUnitNm	= removeCommas(outputCardTable.row(this).data().productQty);
			nextGubun	= outputCardTable.row(this).data().nextGubun;
			workOrdNo	= outputCardTable.row(this).data().workOrdNo;
			seq			= outputCardTable.row(this).data().seq;
			disStatus	= outputCardTable.row(this).data().disStatus;
			
			if(filmLotNo == null){
				filmLotNo = 'test';
			}		
			matrlLotNo = outputCardTable.row(this).data().matrlLotNo;
			//$('#workOrdNoTitle').text(outputCardTable.row(this).data().labelLotNo);
			$('#my-spinner').show();
			
			$('.pushBtn').removeClass('btn-danger');
			$('.pushBtn').addClass('btn-primary');
			$('.pushBtn').attr('style', 'width:65px');
			
			for(var i=0; i<6; i++){
				$('.pushBtn').eq(i).text( $('.pushBtn').eq(i).text().replaceAll('_M', '').replaceAll('_V', '').replaceAll('_MV', '').replaceAll('V', '') );
			}
	...
	$('#outputCardTable2 tbody').on('click', 'tr', function () {
		if ($(this).find('td').eq(0).text() == '합계') {
			return false;
        }
        if ($(this).find('td').eq(0).text() == "데이터가 없습니다.") {            
		...
		if ($(this).hasClass('selected')) {
			...
			$('#outputCardTable2').DataTable().$('tr.selected').removeClass('selected');
			$(this).addClass('selected');
			shipInspIdx = outputCardTable2.row(this).data().idx;
			labelLotNo = outputCardTable2.row(this).data().labelLotNoSeq;	
			$('#productQty').val(outputCardTable2.row(this).data().productQty);
			filmLotNo = outputCardTable2.row(this).data().filmLotNo;
			packUnitNm = removeCommas(outputCardTable2.row(this).data().productQty);		
			nextGubun = outputCardTable2.row(this).data().nextGubun;
			workOrdNo = outputCardTable2.row(this).data().workOrdNo;			
			seq = outputCardTable2.row(this).data().seq;	
			if(filmLotNo == null){
				filmLotNo = 'test';
			}		
			disStatus = outputCardTable2.row(this).data().disStatus;
			matrlLotNo = outputCardTable2.row(this).data().matrlLotNo;
			//$('#workOrdNoTitle').text(outputCardTable2.row(this).data().labelLotNoSeq);
			$('#my-spinner').show();
	...
	function scanWorkOrdOutputSubOrdLotNo() {
		...
		if(activeTab == '001'){
			reInspStatus = '001';
		} else if(activeTab == '002'){
			reInspStatus = '003';
		}
		$.ajax({
	    	url: '<c:url value="/qm/shipInspAdmRead"/>',
	        type: 'GET',
	        data: {
	        	'menuAuth'	 	: 		menuAuth,
	        	'trayNo'		:		function() { return labelLotNo; },
				'reInspStatus'	: 		function() { return reInspStatus;},
				'seq'			:		function() { return seq;},
				'factoryCode'	:		function() { return '003';},
	        ...
	        success: function (res) {
	            ...
	            if (res.result == 'ok') {
	            	...
	            	if (data == null || data == "") {
	            		...
	            		$('#workOrdDate').val(moment(data[0].productDate).format('YYYY-MM-DD'));
	            		productDateValue = $('#workOrdDate').val();
					} else {
						sideView 		= 'edit';					
						workOrdNo 		= data[0].workOrdNo;					
					
						
						$('#workOrdDate').val(moment(data[0].productDate).format('YYYY-MM-DD'));
						$('#inspDate').val(moment(data[0].inspDate).format('YYYY-MM-DD'));						
						$('#userNm').val(data[0].inspChargrNm);
						$('#userNumber').val(data[0].inspChargr);						
						$(".num_only").val("0");						
						$('#faultyCnt').val(addCommas(data[0].faultyCnt));
						$('#dayNightGubun').val(data[0].dayNightCd);
						$('#faultyGubun').val(data[0].faultyGubun);
						$('#inspStatus').val(data[0].inspStatus);
						$('#inspDesc').val(data[0].inspDesc);
						$('#sampleCnt').val(data[0].sampleCnt);
						$('#threeTrayWeight').val(data[0].threeTrayWeight);
						$('#trayWeightDate').val(moment(data[0].trayWeightDate).format("YYYY-MM-DD"));
						
						$('#inspGubun').val(data[0].inspGubun);
						$('#reInspSystem').val(data[0].reInspSystem);
						
						trayFaultSplit = data[0].faultyTrayNo.split(',');
						
						$('.num_only').attr('disabled', true);
						$('#btnSave').attr('disabled', true);
						$('#btnAdd').attr('disabled',true);     
						$('#btnEdit').attr('disabled',false);       
						//삭제버튼비활 $('#btnDel').attr('disabled',false);			
						$("#btnDis").attr("disabled", false);			
						$('#btnAddLot').attr('disabled',false);
						if(data[0].nextGubun == '001'){
							$("#btnReInsp").attr("disabled", false);
							$("#btnReInspCancel").attr("disabled", true);
						} else{
							$("#btnReInsp").attr("disabled", true);
							$("#btnReInspCancel").attr("disabled", false);
						}

						if(data[0].disStatus == '001' || data[0].disStatus == '004'){
							$("#btnDis").attr("disabled", true);
							$('#btnAdd').attr('disabled',true); 
							$('#btnEdit').attr('disabled',true);   
							$("#btnReInsp").attr("disabled", true);  
			...
			complete:function() {
				...
				//출하불량 TB 상세조회
 		        $.ajax({
 		        	url: '<c:url value="/qm/shipInspDtlRead"/>',
 		            type: 'GET',
 		            data: {
 		            	'menuAuth'	 	: 		menuAuth,
 		            	'reInspStatus'	: 		function() { return reInspStatus;},
 		            	'trayNo'		:		function(){return labelLotNo;},		//생산LorNo
 		            },
 		            success: function (res) {
 		                let data = res.data;
 		                if (res.result == 'ok') {
 					...
 					complete:function() {
						...
						$("#btnSizeCondStep").attr("disabled", false);
 		 		    	...
 		 		    	$('.pushBtn').addClass('btn-primary');
 		 		    	if(trayFaultSplit != ""){
 		 		    		$.each(trayFaultSplit, function(index,item) {
 								$('.pushBtn').eq(item-1).removeClass('btn-primary');
 								$('.pushBtn').eq(item-1).addClass('btn-danger');
 							});
	...
	// 재검요청 버튼
	$('#btnReInsp').on('click',function() {
		let reInspStatusValue = '001';
		if(activeTab == '001'){			
			if (!$('#outputCardTable tr').hasClass('selected')) {
				toastr.warning('트레이를 선택해 주세요!');
				return false;
			}
			reInspStatusValue = '001';
		} else if(activeTab == '002'){
			
			if (!$('#outputCardTable2 tr').hasClass('selected')) {
				toastr.warning('트레이를 선택해 주세요!');
				return false;
			}
			reInspStatusValue = '003';
		}

		if(nextGubun == '001'){		

			if(activeTab == '001'){
				if($('#inspStatus').val() =='002'){
					$.ajax({
						url: '<c:url value="qm/shipInspectReInspStatusUpdateFactory1"/>',
			            type: 'POST',
			            data: {		      
				            'trayNo'			: 	labelLotNo,
				            'reInspStatus'		:   '001',
				...
				if($('#inspStatus').val() =='002'){
					$.ajax({
						url: '<c:url value="qm/shipInspectReInspStatusUpdateReturn"/>',
			            type: 'POST',
			            data: {		      
				            'trayNo'			: 	labelLotNo,
				            'reInspStatus'		:   '003',
			...
			toastr.warning('이미 요청이 완료된 트레이 입니다.');
			return false;
		}
	});
	
	//재검요청취소 버튼 클릭 시
	$('#btnReInspCancel').on('click', function() {
		
		if( !$('#inspTab1').hasClass('show') ) {
			toastr.warning('출하검사탭을 선택해주세요');
			return false;	
		}
		if (!$('#outputCardTable tr').hasClass('selected')) {
			toastr.warning('트레이를 선택해 주세요!');
		...
		$.ajax({
			url: '<c:url value="qm/reInspectRequestCancel_F3"/>',
            type: 'GET',
            data: {		      
	            'trayNo' : 	function() { return labelLotNo; }
            },
			...
			success : function(res) {
				...
				if (res.result == "ok") {
					...
					if( data == 'Y' ) {
						toastr.success('재검요청취소되었습니다.');
					} else {
						toastr.warning('재검이 등록되어 있어 삭제할 수 없습니다.');
					}
...
// 		$('#badTotal6').text(addCommas(badTotal6));
		//$('#faultyCnt').val(addCommas(parseInt(metalBadTotal) + parseInt(filmBadTotal) + parseInt(visionBadTotal) + parseInt(badTotal5) ));
		$('#faultyCnt').val(addCommas(parseInt(metalBadTotal) + parseInt(filmBadTotal) ));
	...
	$('#btnSave').on('click', function() {
        ...
        if ($('#appearOutputCnt').val() == '0' || $('#appearOutputCnt').val() == '00') {
            toastr.warning('출하양품수량을 입력해 주세요!');
            $('#appearOutputCnt').focus();
            return false;
        } */
        if (!$.trim($('#workOrdDate').val())) {
            toastr.warning('생산일자를 입력하세요.');
            $('#workOrdDate').focus();
            return false;
        }
        ...
        if (!$.trim($('#inspDate').val())) {
            toastr.warning('검사일자를 입력하세요.');
            $('#inspDate').focus();
            return false;
        }
        if($('#inspStatus').val() == ""){
			toastr.warning("합불판정을 선택해주세요!");
			 $('#inspStatus').focus();
			return false;				
		}
        
        if ( $('#inspStatus').val() != '001' ) {
        	if($('#faultyGubun').val() == ""){
				toastr.warning("부적합유형를 선택해주세요!");
				 $('#faultyGubun').focus();
				return false;				
			}
	        if($('#inspGubun').val() == ""){
				toastr.warning("재검유형를 선택해주세요!");
				 $('#inspGubun').focus();
				return false;				
			}
	        if($('#reInspSystem').val() == ""){
				toastr.warning("재검방법을 선택해주세요!");
				 $('#reInspSystem').focus();
				return false;				
			}
        }
       ...
       /*  if (parseInt(removeCommas($('#appearOutputCnt').val())) > 99999) {
        	toastr.warning('출하 양품수량을 확인해주세요!');
        ...
        //if (trueFalse) {
			...
			var edgeOutputCnt = 0;
			let faultyTrayNo = '';
			let faultyTotalCnt = faultyTotalCntCalc();
			$.each($('.pushBtn'), function() {
				if($(this).hasClass('btn-danger')){
					faultyTrayNo += $(this).text().replaceAll('_M', '').replaceAll('_V', '').replaceAll('_MV', '').replaceAll('V', '') + ",";
				}
			});

			faultyTrayNo = faultyTrayNo.substring(0,faultyTrayNo.length-1);

			if((faultyTotalCnt != 0 && faultyTrayNo == "") && $('#inspStatus').val() == '002'){
				toastr.warning('불량이 발생한 트레이를 선택해주세요.');
				$('#my-spinner').hide();
				return false;
			}
			...
			$.each($('#metalFaultyTbody .num_only'), function() {
				if($(this).val() != '0' && $(this).val() != '00') {
					...
					rowData.trayNo 			= labelLotNo;
					rowData.workOrdNo 		= workOrdNo;
					rowData.seq				= seq;
					rowData.matrlLotNo 		= matrlLotNo;
					rowData.filmLotNo 		= filmLotNo;
					rowData.inspStatus 		= $('#inspStatus').val();
					rowData.inspDate 		= $('#inspDate').val().replace(/-/g,"");
					rowData.faultyTotalCnt 	= faultyTotalCnt;									
					rowData.faultyTrayNo 	= faultyTrayNo;
					rowData.inspChargr 		= $('#userNumber').val();
					rowData.dayNightGubun 	= $('#dayNightGubun').val();
					rowData.majorCd 		= '001';
					rowData.minorCd 		= $(this).parent().find('input[type=hidden]').val();
					rowData.faultyGubun 	= $('#faultyGubun').val();
					rowData.faultyCnt 		= $(this).val().replace(/,/g,"");
					rowData.inspDesc		= $('#inspDesc').val();
					rowData.sampleCnt		= $('#sampleCnt').val();
					rowData.threeTrayWeight	= $('#threeTrayWeight').val();
					rowData.trayWeightDate	= $('#trayWeightDate').val().replace(/-/g,"");
					rowData.productDate		= $('#workOrdDate').val().replace(/-/g,"");
					rowData.inspGubun 		= $('#inspGubun').val();
					rowData.reInspSystem 	= $('#reInspSystem').val();
					if(nextGubun == '' || nextGubun == null){
						rowData.nextGubun		= "001";
					} else{
						rowData.nextGubun		= nextGubun;
					}

					if(activeTab == '001'){
						rowData.reInspStatus  = '001';
					} else if(activeTab == '002'){
						rowData.reInspStatus  = '003';
					}
 ...
 */
            ...
            if(activeTab == '001'){
/*             	if (sideView == 'add'){
        			url = '<c:url value="/qm/shipInspectCreate"/>';
        		} else if (sideView == 'edit'){
        			url = '<c:url value="/qm/shipInspectUpdate"/>';
        		} */

            	url = '<c:url value="/qm/shipInspectUpdate_Fn3"/>';
    		...
    		$.ajax({
				...
				success: function (res) {                                                                  				
					...
					if(res.result == 'ok') {
						if(activeTab == '001'){
							toastr.success('출하검사 등록되었습니다.');
						} else{
							toastr.success('출하재검 등록되었습니다.');
						}
    			...
    			complete:function(){
					...
					if(activeTab == '001'){
	    				...
	    				if($('#inspStatus').val() == '002'){
	    					$('#ReInspConfirmPopupModal').modal({backdrop: 'static'});
		    				$('#ReInspConfirmPopupModal').modal('show');
		    			}	
	    				
					} else if(activeTab == '002'){
					
	    				let rowsOut 	= outputCardTable2.rows('.selected').indexes();		       		
		       			let indexOut	= $('#outputCardTable2').DataTable().row(rowsOut).index();
    		...
    		//포장 boxNo 생성
			if (activeTab == '001' && sideView == "add") {
				let tryPackUrl = null;
//	 			if (sideView == 'add') {
					tryPackUrl = '<c:url value="/po/goodsPackByTray_nF2_Factory1"/>';
//	 			} else if (sideView == 'edit') {
//	 				tryPackUrl = '<c:url value="/qm/shipInspectUpdate"/>';
//	 			}
	    		...
	    		$.ajax({
					...
					data: {
						...
						'ordGubun'		:	function(){ return $('#boxOrdGubunCd').val(); },
						'packUnitNm'	:	function(){ return packUnitNm; },
					},
					success: function (res)   {
						let data = res.data;
						if(res.result == 'ok') {
 ...
 */
		...
		//현재시간이 07:00:00 ~ 18:59:59 이면 전표(생산)일자는 오늘, 주간으로 표시
			...
			//$('#workOrdDate').val(today);
			$('#inspDate').val(today);
			selectBoxAppend(ordGubunCode, "dayNightGubun", "D", "");
		}

		$('#workOrdDate').val(productDateValue);
		$('#userNm').val(userNm);
	...
	$('#btnDis').on('click', function() {
		...
		if(activeTab == '001'){
			if($('#outputCardTable tbody tr').hasClass('selected')=='') {
				toastr.warning('폐기할 항목을 선택해주세요!', '', {timeOut: 5000});
				return false;
			}
		} else if(activeTab == '002'){
			if($('#outputCardTable2 tbody tr').hasClass('selected')=='') {
				toastr.warning('폐기할 항목을 선택해주세요!', '', {timeOut: 5000});
				return false;
			}
		}
		...
		$('#disAppearCheckPopupModal').modal({backdrop: 'static'});
		$('#disAppearCheckPopupModal').modal('show');
	});

	//모달 삭제 버큰 클릭 시
	$('#btnDisAppearCheck').on('click', function(){
		let delReInspStatus = '';
		if(activeTab == '001'){
			delReInspStatus  = '001';
		} else if(activeTab == '002'){
		...
		//삭제
		$.ajax({
			url : '<c:url value="qm/shipInspectDisposal"/>',
			type : 'POST',
			data : {
				'idx'		: function() { return shipInspIdx;		},
				'trayNo'		: function() { return labelLotNo;	},
				'reInspStatus' 	: function() { return delReInspStatus;},
				'workOrdNo'		:	function(){ return workOrdNo; },		
				'ordGubun'		:	function(){ return $('#boxOrdGubunCd').val(); },
				'packUnitNm'	:	function(){ return packUnitNm; },	
				'seq'			:	function(){ return seq;},
				'nextGubun'			:	function(){ return nextGubun;},
				'inspDate'			:	function(){ return $('#inspDate').val().replace(/-/g,"");},
				'inspChargr'			:	function(){ return $('#userNumber').val();},
				'inspStatus'		:	function() {return $('#inspStatus').val();}
			},
			success : function(res) {
				let data = res.result;
				if (res.result == 'ok') {
...
// 		$('#badTotal6').text("0");
    ...
    function viewDisabled(flag){
    	...
    	$('#inspDate').attr("disabled", flag);
    	$('#searchUser').attr("disabled", flag);    	
    	//$("#dayNightGubun").attr("disabled", flag);
    	$('.num_only').attr('disabled', flag);
    	$('.pushBtn').attr('disabled', flag);

		$('#inspGubun').attr('disabled',flag);
		$('#reInspSystem').attr('disabled',flag);
    	$('#faultyGubun').attr('disabled',flag);
    	$('#inspStatus').attr('disabled',flag);
    	$('#inspDesc').attr('disabled',flag);
    	$('#sampleCnt').attr('disabled',flag);
    	$('#threeTrayWeight').attr('disabled',flag);
    	$('#trayWeightDate').attr('disabled',flag);
    }
	...
	$('.num_only').keypress(function (event) { 
		...
		if ($(this).attr("maxlength") < $(this).val().length) {
 		...
 		var arrowKeys = ["ArrowLeft", "ArrowUp", "ArrowRight", "ArrowDown"];
 		
 		//96~105가 키패드 키 코드이다.
 		//if ( !((event.which >= 96 && event.which <= 105) || (event.which >= 48 && event.which <= 57) || event.which == 13 || (event.which >= 37 && event.which <= 40))) {
 			
 		if ( !((event.key >= 0 && event.key <= 9) || event.key == "Enter" || arrowKeys.includes(event.key) )) {
	...
	$('.num_only2').keypress(function (event) { 
...
/* 			var sum1 = parseInt($("#metalMmb").val() || 0);
	...
	function faultyTotalCntCalc(){
		let faultyTotalCnt = 0;
		$.each($('input.num_only'), function(index,item){				
			faultyTotalCnt += parseInt($(this).val() || 0);
		});
		return faultyTotalCnt;
	...
	/* $('#threeTrayWeight').focusout(function() {
		tagId = $(this).attr('id');
	}); */
	...
	$('#number0').on('click', function () {
	    let temp = $('#' + tagId).val();
	    if (temp == "0") {
	        $('#' + tagId).val("0")[0].dispatchEvent(new Event('input', { bubbles: true }));
	    } else {
	        $('#' + tagId).val(temp.concat("0"))[0].dispatchEvent(new Event('input', { bubbles: true }));
	    }
	    //$('#' + tagId).val('123')[0].dispatchEvent(new Event('input', { bubbles: true }));
	...
	$('#number1').on('click', function () {
		let temp = $('#'+tagId).val();
		if (temp == "0"){
			$('#'+tagId).val("1")[0].dispatchEvent(new Event('input', { bubbles: true }));
		} else{
			$('#'+tagId).val(temp.concat("1"))[0].dispatchEvent(new Event('input', { bubbles: true }));
		}
		$(".num_only").trigger("keyup");
	});
	$('#number2').on('click', function () {
		let temp = $('#'+tagId).val();
		if (temp == "0"){
			$('#'+tagId).val("2")[0].dispatchEvent(new Event('input', { bubbles: true }));
		} else{
			$('#'+tagId).val(temp.concat("2"))[0].dispatchEvent(new Event('input', { bubbles: true }));
		}
		$(".num_only").trigger("keyup");
	});
	$('#number3').on('click', function () {
		let temp = $('#'+tagId).val();
		if (temp == "0"){
			$('#'+tagId).val("3")[0].dispatchEvent(new Event('input', { bubbles: true }));
		} else{
			$('#'+tagId).val(temp.concat("3"))[0].dispatchEvent(new Event('input', { bubbles: true }));
		}
		$(".num_only").trigger("keyup");
	});
	$('#number4').on('click', function () {
		let temp = $('#'+tagId).val();
		if (temp == "0"){
			$('#'+tagId).val("4")[0].dispatchEvent(new Event('input', { bubbles: true }));
		} else{
			$('#'+tagId).val(temp.concat("4"))[0].dispatchEvent(new Event('input', { bubbles: true }));
		}
		$(".num_only").trigger("keyup");
	});
	$('#number5').on('click', function () {
		let temp = $('#'+tagId).val();
		if (temp == "0"){
			$('#'+tagId).val("5")[0].dispatchEvent(new Event('input', { bubbles: true }));
		} else{
			$('#'+tagId).val(temp.concat("5"))[0].dispatchEvent(new Event('input', { bubbles: true }));
		}
		$(".num_only").trigger("keyup");
	});
	$('#number6').on('click', function () {
		let temp = $('#'+tagId).val();
		if (temp == "0"){
			$('#'+tagId).val("6")[0].dispatchEvent(new Event('input', { bubbles: true }));
		} else{
			$('#'+tagId).val(temp.concat("6"))[0].dispatchEvent(new Event('input', { bubbles: true }));
		}
		$(".num_only").trigger("keyup");
	});
	$('#number7').on('click', function () {
		let temp = $('#'+tagId).val();
		if (temp == "0"){
			$('#'+tagId).val("7")[0].dispatchEvent(new Event('input', { bubbles: true }));
		} else{
			$('#'+tagId).val(temp.concat("7"))[0].dispatchEvent(new Event('input', { bubbles: true }));
		}
		$(".num_only").trigger("keyup");
	});
	$('#number8').on('click', function () {
		let temp = $('#'+tagId).val();
		if (temp == "0"){
			$('#'+tagId).val("8")[0].dispatchEvent(new Event('input', { bubbles: true }));
		} else{
			$('#'+tagId).val(temp.concat("8"))[0].dispatchEvent(new Event('input', { bubbles: true }));
		}
		$(".num_only").trigger("keyup");
	});
	$('#number9').on('click', function () {
		let temp = $('#'+tagId).val();
		if (temp == "0"){
			$('#'+tagId).val("9")[0].dispatchEvent(new Event('input', { bubbles: true }));
		} else{
			$('#'+tagId).val(temp.concat("9"))[0].dispatchEvent(new Event('input', { bubbles: true }));
		}
		$(".num_only").trigger("keyup");
	});

	$('#number0, #number1, #number2, #number3, #number4, #number5, #number6, #number7, #number8, #number9').on('click', function(){
		if ($('#appearOutputCnt').attr("maxlength") < $('#productQty').val().length) {
	...
	$('#backSpace').on('click', function () {
		...
		$('#'+tagId).val(temp)[0].dispatchEvent(new Event('input', { bubbles: true }));
	...
	//			'appearOutputCnt'		:	parseInt($('#edgeOutputCnt').val() - sum),		//출하 양품 개수				
	//			'appearFaultyCnt'		:	function(){return sum},	//출하 총 불량 개수
	//		},
	//		success: function (res) {
	...
	//			if( res.result == 'ok' ){
	//				if(searchData == true) {
	//					toastr.success("출하검사 불량 수정 되었습니다.");
	//				}
	//				else if(searchData == false){
	//					toastr.success("출하검사 불량 저장 되었습니다. 종물을 등록해 주세요!");
	//				}
...
// 		$('#faultyDiv6').removeClass("d-none");
 ...
 //       	$('#workOrderTable').DataTable().ajax.reload( function () {});
			...
			if( outputCardTable2.data().length == 0 ) {
				if(outputCardTable.data()[0].inspStatus == "001") {
					toastr.warning("이미 합격된 Lot 입니다.");
	    		} else {
	    			toastr.success("스캔되었습니다.");
	    		}
			} else {
				if(activeTab != '002') {
					if(outputCardTable2.data()[0].reInspStatus == "003") {
		    			toastr.warning("출하재검 Lot 입니다.");
		    		} else {
		    			toastr.success("스캔되었습니다.");
		    		}
				} else {
...
//     		$('#workOrdNoScan').focus();
	...
	$('.pushBtn').on('click',function() {
		...
		if($(this).hasClass('btn-primary')) {
			...
			if( this.style.background != null && this.style.background != '' ) {
				let style_danger = 'width:65px;background: '+this.style.background+';border: 1px '+this.style.background+' solid;color: red;';
				$(this).attr('style', style_danger);
			}
			...
			if( this.style.background != null && this.style.background != '' ) {
				let style_danger = 'width:65px;background: '+this.style.background+';border: 1px '+this.style.background+' solid;';
				$(this).attr('style', style_danger);
			}
		}
	...
	$('#inspTab1').on('click',function(){
		$('#inspDiv').removeClass('d-none');
		$('#reInspDiv').addClass('d-none');
		$('#btnReInspCancel').removeClass('d-none');
	...
	$('#inspTab2').on('click',function(){
		$('#inspDiv').addClass('d-none');
		$('#reInspDiv').removeClass('d-none');
		$('#btnReInspCancel').addClass('d-none');
	...
	$('#btnAddLot').on('click',function(){
		$('#lotAdd').val("");
		$('#lotAddModal').modal('show');
	});

	$('#btnModalLotAdd').on('click',function() {
		 if (!$.trim($('#lotAdd').val())) {
		...
		$.ajax({
            ...
            data: {		      
	            'trayNo'			: 	labelLotNo,     
            	'reInspStatus'		:	reInspStatus,
            	'matrlLotNo'		:	$('#lotAdd').val(),
            	'seq'				:	seq
            },
            beforeSend: function() {
            	$('#my-spinner').show();
			...
			success : function(res) {
				...
				$('#lotAddModal').modal('hide');
				$('#my-spinner').hide();
			}
		});
	});

	$('#btnReInspConfirm').on('click',function(){
		$("#btnReInsp").trigger("click");
		if(activeTab == '001'){
			$('#outputCardTable').DataTable().ajax.reload(function(){		       			
       		});	
			$('#outputCardTableRecord').DataTable().ajax.reload( function () {});
	...
	//상단 스크롤 이동 버튼 클릭 시
	function btnScrollTop() {
		let scrollTop = $('#divScrollMove').scrollTop();
    	$('#divScrollMove').scrollTop(scrollTop - 180);
	}

	//하단 스크롤 이동 버튼 클릭 시
    function btnScrollBottom() {
    	let scrollBottom = $('#divScrollMove').scrollTop();
    	$('#divScrollMove').scrollTop(scrollBottom + 180);
	}
		...
		$.ajax({
			...
			success: function (res) {
				...
				if( res.result == 'ok' ) {
					...
					let style_M		= 'width:65px;background: purple;border: 1px purple solid;';
					let style_V		= 'width:65px;background: #f8cbad;border: 1px #f8cbad solid;';
					let style_MV	= 'width:65px;background: #ffd200;border: 1px #ffd200 solid;';
					
					let style_dangerM	= 'width:65px;background: purple;border: 1px purple solid;color: red;';
					let style_dangerV	= 'width:65px;background: #f8cbad;border: 1px #f8cbad solid;color: red;';
					let style_dangerMV	= 'width:65px;background: #ffd200;border: 1px #ffd200 solid;color: red;';
					
					for( var i=0; i < 6; i++){
						
						//기존 색상이 불량인 경우
						if( $('.pushBtn').eq(i).hasClass('btn-danger') ) {
							
							if( $('.pushBtn').eq(i).text() == data[i].trayRank && data[i].incorporationGubun == "Y" &&  data[i].incorporationVisonGubun == "N" ){
								//메탈 혼입인 경우
								$('.pushBtn').eq(i).attr('style', style_dangerM);
								$('.pushBtn').eq(i).text((i+1)+"_M");
								
							} else if( $('.pushBtn').eq(i).text() == data[i].trayRank && data[i].incorporationGubun == "N" &&  data[i].incorporationVisonGubun == "Y" ) {
								//비전 혼입인 경우
								$('.pushBtn').eq(i).attr('style', style_dangerV);
								$('.pushBtn').eq(i).text((i+1)+"_V");
							} else if( $('.pushBtn').eq(i).text() == data[i].trayRank && data[i].incorporationGubun == "Y" &&  data[i].incorporationVisonGubun == "Y" ) {
								//메탈&비전 혼입인 경우
								$('.pushBtn').eq(i).attr('style', style_dangerMV);
								$('.pushBtn').eq(i).text((i+1)+"_MV");
							} else {
								$('.pushBtn').eq(i).attr('style', 'width:65px;');
							}
							...
							if( $('.pushBtn').eq(i).text() == data[i].trayRank && data[i].incorporationGubun == "Y" &&  data[i].incorporationVisonGubun == "N" ){
								//메탈 혼입인 경우
								$('.pushBtn').eq(i).attr('style', style_M);
								$('.pushBtn').eq(i).text((i+1)+"_M");
							} else if( $('.pushBtn').eq(i).text() == data[i].trayRank && data[i].incorporationGubun == "N" &&  data[i].incorporationVisonGubun == "Y" ) {
								//비전 혼입인 경우
								$('.pushBtn').eq(i).attr('style', style_V);
								$('.pushBtn').eq(i).text((i+1)+"_V");
							} else if( $('.pushBtn').eq(i).text() == data[i].trayRank && data[i].incorporationGubun == "Y" &&  data[i].incorporationVisonGubun == "Y" ) {
								//메탈&비전 혼입인 경우
								$('.pushBtn').eq(i).attr('style', style_MV);
								$('.pushBtn').eq(i).text((i+1)+"_MV");
							} else {
								$('.pushBtn').eq(i).attr('style', 'width:65px;');
							}
	...
	*/
	...
	//입력 완료 벼튼 클릭 시
	$('#btnInputCompletion').on('click', function() {
		
		/* if( outputCardTable.rows('.selected').data().length != 0 ) {
			funThreeTrayInspection(outputCardTable.rows('.selected').data()[0].goodsCd, $('#trayWeightDate').val(), $('#threeTrayWeight').val() );			
		} */
		
		if( activeTab == "001" && outputCardTable.rows('.selected').data().length != 0 ) {
			funThreeTrayInspection(outputCardTable.rows('.selected').data()[0].goodsCd, $('#trayWeightDate').val(), $('#threeTrayWeight').val() );			
		}
		if( activeTab == "002" && outputCardTable2.rows('.selected').data().length != 0 ) {
			funThreeTrayInspection(outputCardTable2.rows('.selected').data()[0].goodsCd, $('#trayWeightDate').val(), $('#threeTrayWeight').val() );			
		}
		

		let faultyArray = new Array();
		
		//----------모두 0이거나 잘못된 데이터 값이 있는 경우 처리X----------//
		let numCheck = false;
		let numCount = 0;
		$.each($('input.num_only'), function(index,item) {
			if (!$.trim($(this).val())) {
		...
		if (numCount == 0) {
			autoSelect("", '001', "", "");
			toastr.success('모든 값이 0입니다.<br>합격처리 되었습니다.');
			return false;
		}
		//----------모두 0이거나 잘못된 데이터 값이 있는 경우 처리X----------//
		...
		$.each($('#metalFaultyTbody .num_only'), function() {
			if(  /^\d*[1-9]/.test( $(this).val() ) ) {
				let obj = new Object();
				obj.baseCd = $(this).attr("id").replace("metal","");
				obj.etc1 = "001";
				obj.value = $(this).val().replace("metal","");
				
				faultyArray.push(obj);
			}
		});
		//----------원자재성 불량----------//
		...
		$.each($('#filmFaultyTbody .num_only'), function() {
			if(  /^\d*[1-9]/.test( $(this).val() ) ) {	
				let obj = new Object();
				obj.baseCd = $(this).attr("id").replace("film","");
				obj.etc1 = "002";
				obj.value = $(this).val();
		...
		//systemCommonCode : etc1 : 발생구분(원자재성&공정성) | etc2 : 순번 | etc3 : 불량구분(Major&Minor) | etc4 : 재검구분(출하재검&부분재검&전수재검) | etc5 : 검사방법(재검사(우선)&재작업) | etc6 : 동일불량합격수량 | etc7 : 이종불량합격수량
		//원자재성&공정성 불량 중 Major불량이 있는 경우 우선으로 처리함
		
		let majorCount		= 0;
		let minorCount		= 0;
		let majorQtyStorage	= 0;	//Major불량에서 사용
		let minorQtyStorage	= 0;	//Minor불량에서 사용
			
		//----------Major 불량인 경우----------//
		for(var i=0; i < faultyArray.length; i++) {
			
			let faultybaseCd	= faultyArray[i].baseCd;
			let faultyQty		= parseInt(faultyArray[i].value);
			let index 			= systemCommonCode.findIndex(obj => obj.baseCd == faultybaseCd);
			let faulty 			= systemCommonCode[index];
			...
			if( faulty.etc3 == '001' ) {
				...
				if(faulty.etc4 == '001') {
					...
					//동일불량합격 수량 넘는지 확인
					if( parseInt(faulty.etc6) < faultyQty ) {
						autoSelect(faulty.etc3, '002', faulty.etc4, faulty.etc5);
						break;
					} else {
						majorQtyStorage += faultyQty;
					}
					//동일불량합격 수량 넘는지 확인
			...
			//이종불량합격수량 넘는지 확인
			if( 1 < majorCount ) {
				if( parseInt(faulty.etc7) < majorQtyStorage ) {
					autoSelect(faulty.etc3, '002', faulty.etc4, faulty.etc5);
					break;
				} else {
					autoSelect("", '001', "001", "");
		...
		if(majorCount == 0) {
			for(var i=0; i < faultyArray.length; i++) {
				...
				//이종불량합격수량 넘는지 확인
				if( 1 < minorCount ) {
					if( parseInt(faulty.etc7) < minorQtyStorage ) {
						autoSelect(faulty.etc3, '002', faulty.etc4, faulty.etc5);
						break;
					} else {
						autoSelect("", '001', "001", "");
	...
	function autoSelect(faultyGubun, inspStatus, inspGubun, reInspSystem) {
		$('#faultyGubun').val(faultyGubun);		//부적합 유형(Major&Minor)
		$('#inspStatus').val(inspStatus);		//합불판정(합격&불합격)
		$('#inspGubun').val(inspGubun);			//재검유형(출하재검&부분재검&전수재검)
		$('#reInspSystem').val(reInspSystem);	//검사방법(재검사&재작업)
	}
	
	
	//총 출하불량수량
	let metalInput = document.querySelectorAll("[id*=metal][type=number]");
	let filmInput = document.querySelectorAll("[id*=film][type=number]");

	metalInput.forEach((inputBox) => {
		inputBox.addEventListener('input', (event) => {
			const currentValue = event.target.value;
			$('#metalBadTotal').text(sudelFun("A"));
			
			$('#faultyCnt').val( sudelFun("A") + sudelFun("B") );
		});
	});
	
	filmInput.forEach((inputBox) => {
		inputBox.addEventListener('input', (event) => {
			const currentValue = event.target.value;
			$('#filmBadTotal').text(sudelFun("B"));
			
			$('#faultyCnt').val( sudelFun("A") + sudelFun("B") );
		});
	});
	
	function sudelFun(gubun) {
		let sudelSum = 0;
		if( gubun == "A" ) {
			$("[id*=metal][type=number]").each(function() {
			  sudelSum += parseInt($(this).val()) || 0;
			});	
		} else {
			$("[id*=film][type=number]").each(function() {
			  sudelSum += parseInt($(this).val()) || 0;
			});
		}
		return sudelSum;
	...
	var inputBox = document.querySelector('#threeTrayWeight');

	inputBox.addEventListener('input', (event) => {
		var currentValue = event.target.value;
		currentValue = currentValue.replaceAll('.', '');
		//console.log(convertNumber(currentValue));
		$('#threeTrayWeight').val( convertNumber( currentValue ) );
	});
	
	//정수입력하면 3자리 끊어서 소수점 반환 함수
	function convertNumber(num) {
		...
		if ( len < 4 || len == "" ) {
			...
			var lastTwoDigits = str.slice(-3);
			var restDigits = str.slice(0, -3);
			return restDigits + '.' + lastTwoDigits;
		}
	}

	
	function trayWeightDateCall() {
		//저장된 날짜 데이터 조회
   		$.ajax({
            ...
            success: function (res) {
                ...
                if(res.result == 'ok') {
					...
					selectBoxAppend(dateListCode, "trayWeightDate", lastDate, "");
	...
	function funThreeTrayInspection(goodsCd, trayWeightDate, threeTrayWeight) {
		//저장된 날짜 데이터 조회
   		$.ajax({
        	url: '<c:url value="bm/threeTrayInspection"/>',
            type: 'GET',
            async: false,
            data: {
            	'menuAuth'			: menuAuth,
            ...
            success: function (res) {
                ...
                if(res.result == 'ok') {
                	...
                	data = data[0];
                	if( data != null ) {
                		let condition = "-" + data.referenceMinus + "[" + data.referenceValue + "] +" + data.referencePlus ;

                    	if( data.referenceValue == "" || data.referenceValue == null ) {
                    		toastr.warning('(3Tray)등록된 코드가 없습니다.');	
                    	} else if( data.referencePlus < parseFloat(threeTrayWeight) && data.referenceMinus < parseFloat(threeTrayWeight) ) {
                    		toastr.warning('3Tray중량은 초과입니다.<br>조건: '+condition+'');
                    	} else if( data.referencePlus > parseFloat(threeTrayWeight) && data.referenceMinus > parseFloat(threeTrayWeight) ) {
                    		toastr.warning('3Tray중량은 미달입니다.<br>조건: '+condition+'');	
                    	} else {
                    		toastr.success('3Tray중량은 정상입니다.<br>조건: '+condition+'');
                    	}                		
                	} else {
                		toastr.warning('불러올 코드(3Tray)가 없습니다.');	
                	}
