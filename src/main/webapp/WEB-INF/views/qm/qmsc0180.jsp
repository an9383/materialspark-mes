				...
				<div class="card-body" style="padding-top:5px; padding-bottom:5px;">
					<div class="row" style="position:relative">
						...
						<div class="row">
							...
							<label class="input-label-sm ml-1 touch6">생산일</label>
							<input class="form-control touch6" style="width:120px;" type="date" id="chooseDateFrom" name="chooseDateFrom" />
							
							<label class="input-label-sm ml-2 mr-2 touch6">설비명</label>
							<select class="custom-select custom-select-lg w-120 touch6" id="equipCd" style="height:30px;">
							</select>
						...
						<button type="button" class="btn btn-primary float-right mr-5 ml-2 touch6" id="btnWorkOrdRetv">전체조회</button>
							...
							<tr>
								...
								<td colspan="3">
									<div style="display: flex;flex-wrap: nowrap;justify-content: center;">
										<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:60px;"disabled>1</button>
										<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:60px;"disabled>2</button>
										<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:60px;"disabled>3</button>
										<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:60px;"disabled>4</button>
										<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:60px;"disabled>5</button>
										<button type="button" class="btn btn-primary float-left mr-2 touch6 pushBtn" style="width:60px;"disabled>6</button>
...
<!-- 									<li class="nav-item"><a class="nav-link" data-toggle="tab" id="tab6">...</a></li> -->
					...
					<div class="card-body p-0">
						...
						<button type="button" class="btn btn-primary float-right mr-2 touch6" id="btnReInsp" disabled>재검요청</button>
					</div>
					<br>
	    ...
	    <div class="modal-dialog modal-lg" role="document" style="width: 85vh;">
	        <div class="modal-content" style="width: 60%; margin-left: 30%; margin-top: 20%;" id="workerChangeContent">
	            ...
	            <div class="modal-body">
					...
					<div class="mb-2 row">
						<div class="table-responsive">
							<table id="lotAddTable" class="table table-bordered">
								<thead class="thead-light">
									<tr>
										<th style="min-width:70px">작지번호</th>
										<th style="min-width:80px">메탈LOT</th>
										<th style="min-width:45px">필름LOT</th>										
									</tr>
								</thead>
							</table>
						</div>
...
// 	faultyHdCreate6();
    ...
    let outputCardTable = $('#outputCardTable').DataTable({
		...
		pageLength: 99999,
        ajax: {
			url: '<c:url value="/qm/shipInspSelectList"/>',
			type: 'GET',
			async: false,
			data: {
				'menuAuth'			: 	menuAuth,
        		...
        		'equipCd'			:	function(){return $('#equipCd').val();}
...
// 			var appearTotalRatio = 	0;
    ...
    let outputCardTable2 = $('#outputCardTable2').DataTable({
		...
		pageLength: 99999,
        ajax: {
			url: '<c:url value="/qm/reShipInspSelectList2"/>',
			type: 'GET',
			async: false,
			data: {
				'menuAuth'			: 	menuAuth,
	            ...
	            if (res.result == 'ok') {
	            	if (data == null || data == "") {
						...
						if($('#inspStatus').val() == '001'){
							$('#faultyMajor').addClass('d-none');							
						} else {		
							$('#faultyMajor').removeClass('d-none');							
						}
						
						if(data[0].disStatus == '001'){
							$("#btnDis").attr("disabled", true);
							$('#btnAdd').attr('disabled',true); 
							$('#btnEdit').attr('disabled',true);   
							$("#btnReInsp").attr("disabled", true);  
		...
		if(nextGubun == '001'){		
			...
			if(activeTab == '001'){
				if($('#inspStatus').val() =='002'){
					$.ajax({
						url: '<c:url value="qm/shipInspectReInspStatusUpdate"/>',
			            type: 'POST',
			            data: {		      
				            'trayNo'			: 	labelLotNo,
				            'reInspStatus'		:   '001',
...
/*             	if (sideView == 'add'){
			...
			if (activeTab == '001' && sideView == "add") {
				let tryPackUrl = null;
//	 			if (sideView == 'add') {
					tryPackUrl = '<c:url value="/po/goodsPackByTray_nF2"/>';
//	 			} else if (sideView == 'edit') {
...
//     		$('#workOrdNoScan').focus();
	...
	$('#btnModalLotAdd').on('click',function() {
		 if (!$('#lotAddTable tbody tr').hasClass('selected')) {
		...
		$.ajax({
			url: '<c:url value="qm/getAddLotNoUpdate"/>',
            type: 'POST',
            data: {		      
	            'trayNo'			: 	labelLotNo,     
            	'reInspStatus'		:	reInspStatus,
            	'matrlLotNo'		:	mainLotNo,
            	'filmLotNo'			:	subLotNo,
            	'seq'				:	seq
            },
            beforeSend: function() {
	...
	function selectLotAddTable()
	{
		...
		if(activeTab == '001'){
	     ...
	     if(lotAddTable == null || lotAddTable == undefined)   {
		     
	    	 lotAddTable = $('#lotAddTable').DataTable({
				language: lang_kor,
				lengthChange : false,
				paging: true,
				info: true,
				...
				pageLength: 20,
	            ajax: {
	                url: '<c:url value="/qm/getAddLotNoList"/>',
	                type: 'GET',
	                data: {
	                	'trayNo'	 	: 		function(){return labelLotNo;},
	                	'reInspStatus'	: 		function(){return reInspStatus;},
	    	 ...
	    	 $('#lotAddTable tbody').on('click', 'tr', function () {
	    	   		if ($(this).hasClass('selected')) {
	    				
	    			} else {
	    				$('#lotAddTable').DataTable().$('tr.selected').removeClass('selected');
	    				$(this).addClass('selected');
	    				mainLotNo = lotAddTable.row(this).data().matrlLotNo;			
	    				subLotNo = lotAddTable.row(this).data().filmLotNo;
	    				
	    			}
	    	});
	    		
		} else{
			$('#lotAddTable').DataTable().ajax.reload();
		}
