	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">초중종물검사현황</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
				...
				<div class="table-responsive" style="padding-bottom: 10px;">
					<table id="stateTable" class="table table-bordered" style="width: 150%;">
						<thead class="thead-light">
							<tr>
								<th rowspan="2" style="min-width:120px;">작업지시</th>
								<th rowspan="2" style="min-width:50px;">호기</th>
								<th rowspan="2" style="min-width:50px;">생산일자</th>
								<th rowspan="2" style="min-width:100px;">주재료</th>
								<th rowspan="2" style="min-width:80px;">제품명</th>
								<th rowspan="2" style="min-width:50px;">재질</th>
		                        <th rowspan="2" style="min-width:55px;">측정시간</th>
		                        
		                        <th colspan="3">Welding Part</th>
...
//             	'ordDate'			:	function() { return serverDateFrom.replace(/-/g, ''); },
        ...
        columns: [
	        ...
	        { render: function(data, type, row, meta) {
	            	if( row['stepCd'] != "000" ) {
	            		//return (parseFloat(row['wpRightAlwnce']) - parseFloat(row['wpLeftAlwnce'])).toFixed(2);
	            		return Math.abs( (parseFloat(row['wpLeftAlwnce']) - parseFloat(row['wpRightAlwnce'])).toFixed(2) );
	            	} else{
	            		return "-";
	            	}
    	    	}
            ...
            { data: 'thEdgeResult',
        		render: function(data, type, row) {
        			if (data=='001') {
		    			...
		    			var rtn='<label style="background : red; color : white; width : 100%;">불합격</label>';
		    			return rtn;
	            	} else {
	            		return ''
		            }
			    ...
			    customize: function ( xlsx ) {
			    	...
			    	if( factoryCode == 001 || factoryCode == 002 ) {
				    	...
				    	$('row', sheet).each( function (k, v, index) {
							...
							} else if( termText == "초물" || termText == "중물" || termText == "종물" ) {
								//조건이 아닌 경우 색상 표시
...
// 		console.timeEnd('time');
	...
	if( factoryCode == "003" ){
		stateTable.column( 9 ).visible( false );
		stateTable.column( 24 ).visible( false );
		stateTable.column( 20 ).visible( false );
	}
