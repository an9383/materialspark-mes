...
<div class="page-wrapper" id="page-wrapper">
	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">Leakage Test 결과등록</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
	<div class="container-fluid" id="main">
			...
			<div class="left-list left-sidebar" id="left-list" style="width: 35%; padding-bottom:0px;">
				<div class="card">
					...
					<div class="table-responsive">
						<table id="reliabilityLeakageTable" class="table table-bordered">
					...
					<div class="tab-pane fade active show" id="tab1">
						<div class="p-1 mt-1 row">
							...
							<div class="row mr-6" style="width:800px;">
								<button type="button" class="btn btn-primary " id="" onclick="leakageTestMake()">기본생성</button>
						...
						<form id="form">
							<div class="table-responsive" id="tabTable1">
								<table class="table table-bordered d-none" id="tableA">
									<colgroup>
										<col width="10%">
										<col width="5%">
										<col width="15%">
		...
		<!-- 						<button type="button" class="btn btn-success float-left mr-1" id="" onclick="">도움말</button> -->
								...
								<input multiple="multiple" type="file" class="btn btn-primary custom-file-input" id="btnMultiUpload" name="multifile" style="display: none" accept="image/png, image/jpg, image/jpeg, image/tif, image/tiff, image/raw, image/bmp, image/rle, image/heic">
							</div>
...
<script>
	...
	let menuAuth = 'qmsc3280';
	let currentHref = 'qmsc3280';
	let currentPage = $('.'+currentHref).attr('id');
	$('#'+currentPage).closest('.has-child','li').addClass('has-open has-active');
	$('#'+currentPage).closest('.menu-item').addClass('has-active');
	$(document).attr("title","Leakage Test 결과등록");
...
//	$('#leakageDate').val(leakageDateVal);
	...
	/*
		reliabilityLeakageTable = 왼쪽 mainTable
		tableA = 실링 테이블
		tableB = 침투액 테이블
		tableC = 파괴 테이블
	*/
	...
	var reliabilityLeakageTableIdx = ''; 	//테이블 아이디 값
	...
	let reliabilityLeakageTable = $('#reliabilityLeakageTable').on( 'error.dt', function ( e, settings, techNote, message ) {
		if(techNote == 7) {
	...
	reliabilityLeakageTable.on('draw.dt', function(){
		$('#my-spinner').hide();
	});


	$('#reliabilityLeakageTable').DataTable().$('tr.selected').removeClass('selected');
	...
	//왼쪽 mainTable 클릭 시
	$('#reliabilityLeakageTable tbody').on('click', 'tr', function(){
		...
		leakDate = reliabilityLeakageTable.row(this).data().testDate;
		leakSeq = reliabilityLeakageTable.row(this).data().testSeq;
		regDate = reliabilityLeakageTable.row(this).data().regDate;
...
//		$('#leakageDate').val(moment(leakDate).format("YYYY-MM-DD"));
		...
		let tableA = $('#tableA').DataTable({
			...
			drawCallback : function() {
				$('#tableA > tbody > tr').each(function(index, item) {
					var tIdx = index;
				
					if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).text() == 'NG'){
						$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).attr('style','background:red;');
					} else if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).text() == '無'){
						$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).attr('style','background:yellow;');
						$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(4).attr('style','background:yellow;');
					}
					if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).text() == 'NG'){
						$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).attr('style','background:red;');
					} else if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).text() == '無') {
						$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).attr('style','background:yellow;');
					$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(6).attr('style','background:yellow;');
					}
		...
		photoFileNo = leakDate+'_'+'001'+'_'+leakSeq;
			...
			},
			{
				...
				render : function(data, type, row, meta) {
					if (data == null) {
						...
						html += '		<div class="custom-file" style="height: 150px;">';
						html += '			<img alt="등록된 사진이 없습니다." id="imageFile" src="qm/downloadDtlFile?testDate='+leakDate+'&testSeq='+leakSeq+'&admRegDate='+regDate+'&fileSeq='+row['fileSeq']+'&leakDstryGubun=001&fileOriginName='+row['fileOriginName']+'" name="imageFile1" style="width: 100%; height: 100%">';
						html += '		</div>';
						html += '		<div class="form-group" id="fileTag" name="fileTag" style="height: 20px;">';
						html += '			<a id="fpHref'+meta.row+'" href="qm/downloadDtlFile?testDate='+leakDate+'&testSeq='+leakSeq+'&fileSeq='+row['fileSeq']+'&leakDstryGubun=001&fileOriginName='+row['fileOriginName']+'"><span style="font-size: 14px;" id="fpFn"></span> <span style="font-size: 10px; vertical-align: bottom;" id="imageFileSize">'+data+'</span></a>';
						html += '		</div>';
						html += '	</div>';
						return html;
					}
...
//				$('#photoDtlTable').DataTable().$('tr.selected').removeClass('selected');
				$(this).addClass('selected');
//					idxVal = String($('#photoDtlTable').DataTable().row(this).data().idx);
	...
	//기본생성
	function leakageTestMake(){
		
		sideView = 'add';

		$('#tableB').removeClass('d-none');
		$('#tableA').addClass('d-none');
		
		$('#my-spinner').show();
		 setTimeout(function(){
			var leakageTestList = new Array();
			
			$('#tableB > tbody > tr').each(function(index, item) {
				var idxVal = index;
				var rowData = new Object();
				rowData.testDate = chooseDate2.replace(/-/g,'');
				rowData.equipNm = $('#tableB tbody tr').eq(idxVal).eq(0).find('td').eq(2).text();
				rowData.gubunDay = $('#tableB tbody tr').eq(idxVal).eq(0).find('td').eq(3).text();
				rowData.day_0_1 = $('#tableB tbody tr').eq(idxVal).eq(0).find('td').eq(4).text();
				rowData.gubunNight = $('#tableB tbody tr').eq(idxVal).eq(0).find('td').eq(5).text();
				rowData.night_0_1 = $('#tableB tbody tr').eq(idxVal).eq(0).find('td').eq(6).text();
				rowData.leakDstryGubun = '001';
...
//				for (var pair of formData.entries()) {console.log(pair[1]); }
				...
				$.ajax({
					...
					success : function(res) {
						...
						$('#my-spinner').hide();
						$('#reliabilityLeakageTable').DataTable().ajax.reload();
					}
				});
//			}); 
		}, 200); 
	...
	$(document).on("click", "#applyLeakageTest", function(){
			...
			if(chkVal == ''){
				$('#tableA > tbody > tr').each(function(index, item) {
					var tIdx = index;
					if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(0).children().is(':checked')){
							$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).text('OK');
							$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).attr('style','');
							$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(4).text('Y');
							$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(4).attr('style','');
							$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).text('OK');
							$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).attr('style','');
							$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(6).text('Y');
							$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(6).attr('style','');
					}
				});
			}else if(chkVal == '001'){
				if(!($('input[type=radio][name=dayNGubunList]:checked').val() == 'A')){
					$('#tableA > tbody > tr').each(function(index, item) {
						var tIdx = index;
						if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(0).children().is(':checked')){
							if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(dd).text() == 'OK'){
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(dd).text('NG');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(dd).attr('style','background:red;');
							} else {
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(dd).text('OK');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(dd).attr('style','');
							}
							if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(moo).text() == '無'){
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(moo).text('Y');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(moo).attr('style','');
							}
						}
					});
				} else {	//전체
					$('#tableA > tbody > tr').each(function(index, item) {
						var tIdx = index;
						if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(0).children().is(':checked')){
							if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).text() == 'OK'){
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).text('NG');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).attr('style','background:red;');
							} else {
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).text('OK');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).attr('style','');
							}
							if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(4).text() == '無'){
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(4).text('Y');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(4).attr('style','');
							}
							
							if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).text() == 'OK'){
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).text('NG');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).attr('style','background:red;');
							} else {
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).text('OK');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).attr('style','');
							}
							if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(6).text() == '無'){
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(6).text('Y');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(6).attr('style','');
							}
						}
				...
				if(!($('input[type=radio][name=dayNGubunList]:checked').val() == 'A')){
					$('#tableA > tbody > tr').each(function(index, item) {
						var tIdx = index;
						if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(0).children().is(':checked')){
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(dd).text('無');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(moo).text('無');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(dd).attr('style','background:yellow;');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(moo).attr('style','background:yellow;');
						}
					});
				} else {	//전체
					$('#tableA > tbody > tr').each(function(index, item) {
						var tIdx = index;
						if($('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(0).children().is(':checked')){
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).text('無');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(4).text('無');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).text('無');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(6).text('無');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(3).attr('style','background:yellow;');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(4).attr('style','background:yellow;');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(5).attr('style','background:yellow;');
								$('#tableA tbody tr').eq(tIdx).eq(0).find('td').eq(6).attr('style','background:yellow;');
	...
	$('#leakageTestCreate').on('click', function(){
		...
		setTimeout(function(){
			...
			$('#tableA > tbody > tr').each(function(index, item) {
				...
				rowData.equipNm = $('#tableA tbody tr').eq(idxVal).eq(0).find('td').eq(2).text();
				rowData.gubunDay = $('#tableA tbody tr').eq(idxVal).eq(0).find('td').eq(3).text();
				rowData.day_0_1 = $('#tableA tbody tr').eq(idxVal).eq(0).find('td').eq(4).text();
				rowData.gubunNight = $('#tableA tbody tr').eq(idxVal).eq(0).find('td').eq(5).text();
				rowData.night_0_1 = $('#tableA tbody tr').eq(idxVal).eq(0).find('td').eq(6).text();
...
//				  console.log($('#photoDtlTable').DataTable().row(index).data());
	...
	$(document).on('change','.custom-file-input',function(e){
		...
		setTimeout(function() {
			...
			formData.append("leakDstryGubun", '001');
...
//			async: false,
	...
	$('#reliabilityLeakageTable_length').html(html1);
	...
	$('#btnRetv').on('click', function() {
		...
		if (dateDiff > 31) {
  		...
  		reliabilityLeakageTableIdx ='';
		$('#photoDtlTable tbody').empty();
		countPhoto = 0;
  		$('#reliabilityLeakageTable').DataTable().ajax.reload();
	});
	...
	$('#btnDel').on('click', function(){
		if($('#reliabilityLeakageTable').DataTable().$('tr').is('.selected')){
