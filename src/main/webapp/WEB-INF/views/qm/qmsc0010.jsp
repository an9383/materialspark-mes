		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="layerPopUpLabel">불량코드등록</h5>
	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">불량정보관리</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
		...
		<div class="row table-wrap-hid">
			...
			<div class="left-list left-30" id="left-30" style="width: 28%;">
				<div class="card">
					...
					<div class="table-responsive">
						<table class="table table-bordered" id="faultyCommonGroupCd">
							<thead class="thead-light">
								<tr>
									<th style="width: 20px" id="useYnCd">사용여부</th>
									<th style="width: 20px" id="baseGroupCd">불량코드</th>
									<th style="width: 100px" id="baseGroupCdNm">불량코드명</th>
								</tr>
							</thead>
						</table>
					</div>
			...
			<div class="right-list right-70" id="myrSidenav" style="width: 71%;">
				<div class="card" id="formBox">
					<form id="form">
						<div class="table-responsive">
							<table class="table table-bordered" id="faultyCommonCd">
								<thead class="thead-light">
									<tr>
...
<script>
	...
	let faultyCommonGroupCd = $('#faultyCommonGroupCd').on( 'error.dt', function ( e, settings, techNote, message ) {
	    	if(techNote == 7) {
        ...
        pageLength: 20,
		'ajax': {
			url: '<c:url value="qm/faultyCommonGroupDataList"/>',
			type: 'GET',
			data: {
				'menuAuth'	 	: 		menuAuth,
			},
	...
	var htmlGroup = '<button class="btn btn-primary float-left mr-2" type="button" id="baseGroupAdd">불량코드등록</button>&nbsp;<button class="btn btn-primary float-left mr-2" type="button" id="baseGroupEdit">수정</button>';
	$('#faultyCommonGroupCd_length').html(htmlGroup);
	...
	var baseGroupCd = "0";
	$.fn.dataTable.ext.errMode = 'none';
	let faultyCommonCd = $('#faultyCommonCd').on( 'error.dt', function ( e, settings, techNote, message ) {
	    	if(techNote == 7) {
		    	toastr.error("로그인 세션이 만료 되었습니다.<br/>재로그인 해 주세요.", '', {timeOut: 5000});
		       	location.href = "/";
			}
        ...
        pageLength: 20,	
		'ajax': {
				url: '<c:url value="qm/faultyCommonCodeDataList"/>',
				type: 'GET',
				data: {
					'menuAuth'	 	: 		menuAuth,
	...
	$('#faultyCommonCd_length').html(htmlCd);
   	...
   	$('#baseGroupEdit').click(function () {
   		var data = faultyCommonGroupCd.row('.selected').data();
   		if(data == null || data == undefined ) {
   			toastr.warning("수정할 불량코드들 선택해 주세요.");
   		} else {
   	...
   	$('#baseCdAdd').click(function () {
   		var data = faultyCommonGroupCd.row('.selected').data();
   		if(data == null || data == undefined ) {
   			toastr.warning("등록할 불량코드 목록을 먼저 선택해 주세요.");
   		} else {
	   		...
	   		//기초코드상세 코드 조회
	   		$.ajax({
	        	url: '<c:url value="qm/readCommonSeq"/>',
	            type: 'GET',
	            data: {
	            	'menuAuth'	 	: 		menuAuth,
   	...
   	$('#baseCdEdit').click(function () {
   		var dataGrp = faultyCommonGroupCd.row('.selected').data();
   		if(dataGrp == null || dataGrp == undefined ) {
   			toastr.warning("수정할 불량코드들 먼저 선택해 주세요.");
   			return;
   		}
   		var data = faultyCommonCd.row('.selected').data();
   		if(data == null || data == undefined ) {
	...
	// 불량그룹 상세 정보 보기
	$('#faultyCommonGroupCd tbody').on('click', 'tr', function () {

        if ( $(this).hasClass('selected') ) {
            $(this).removeClass('selected');
        }
        else {
        	$('#faultyCommonGroupCd').DataTable().$('tr.selected').removeClass('selected');
            $(this).addClass('selected');
            
	        baseGroupCd = faultyCommonGroupCd.row( this ).data().baseGroupCd;
		    console.log("baseGroupCd = " + baseGroupCd);
	   		$('#faultyCommonCd').DataTable().ajax.reload( function () {});
        }
   	});
       
	// 불량코드선택
	$('#faultyCommonCd tbody').on('click', 'tr', function () {
        if ( $(this).hasClass('selected') ) {
            $(this).removeClass('selected');
        }
        else {
        	$('#faultyCommonCd').DataTable().$('tr.selected').removeClass('selected');
            $(this).addClass('selected');
        }
   	});
	...
	function baseGroupCdProc(puFlag)
	{
	    if ( !$.trim($('#puBaseGroupCd').val()) ) {
	        toastr.warning('불량코드를 입력해 주세요.');
	        $('#puBaseGroupCd').focus();
	        return false;
	    }
	    if ( !$.trim($('#puBaseGroupCdNm').val()) ) {
	        toastr.warning('불량코드명을 입력해 주세요.');
	    ...
	    if(puFlag == "groupCdAdd") {
	    	groupUrl = '<c:url value="qm/faultyCommonGroupCreate"/>';
	    	groupMsg = '불량코드가 등록 되었습니다.';
	    } else {
	    	groupUrl = '<c:url value="m/faultyCommonGroupUpdate"/>';
	    	groupMsg = '불량코드가 수정 되었습니다.';
	    }
	    ...
	    $.ajax({
	        ...
	        success: function (res) {
	            ...
	            if (res.result == 'ok') {
		            ...
		            $('#faultyCommonGroupCd').DataTable().ajax.reload(); //등록 후 테이블 리로드	            	
	            	$('#layerPopUpModal').modal('hide');
	            } else if(res.result == 'exist') {
	            	toastr.warning("이미 등록되어 있는 코드번호 입니다.");
	            	$('#layerPopUpModal').modal('hide');
	            	$('#faultyCommonCd').DataTable().ajax.reload(); //등록 후 테이블 리로드		            	
	            } else {
	                toastr.error(res.message, '', {timeOut: 5000});
	            }
	        },
	...
	function baseCdProc(puFlag)
	{
	    ...
	    var cdMsg;
	    if(puFlag == "cdAdd") {
	    	cdUrl = '<c:url value="qm/faultyCommonCodeCreate"/>';
	    	cdMsg = '불량코드가 등록 되었습니다.';
	    } else {
	    	cdUrl = '<c:url value="qm/faultyCommonCodeUpdate"/>';
	    	cdMsg = '불량코드가 수정 되었습니다.';
	    }
	    $.ajax({
	        url: cdUrl,
	        type: 'POST',
   	...
   	{
   	   	var html;
   	   	if(flag == "groupCdAdd" || flag == "groupCdEdit") { //불량코드수정
			html = '<tr>';
			html += '<td class="align-middle" style="width: 80px">불량코드</td>';
			html += '<td style="width: 100px"><input type="text" class="form-control" id="puBaseGroupCd" name="puBaseGroupCd" disabled></td>';	
			html += '</tr>';
			html += '<tr>';
			html += '<td class="align-middle"  style="width: 80px">*불량코드명</td>';
