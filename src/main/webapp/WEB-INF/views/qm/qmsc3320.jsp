	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">외관검사(6층)</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
			...
			<div class="right-list right-sidebar" id="myrSidenav" style="width:35%; padding-top:0px; padding-bottom:0px;">
				...
				<div class="col-sm-12" style="display: flex;align-items: baseline;">
					...
					<div class="col-md-8" style="display: flex;justify-content: flex-end;">
						...
						<button type="button" class="btn btn-primary float-right touch6" id="btnEdit" disabled>입력</button>
					
						<button type="button" class="btn btn-danger float-left touch6 ml-1" id="btnDel" disabled>삭제</button>
						<button type="button" class="btn btn-success float-right touch6 d-none" style="margin-right:5px;" id="btnPrint" disabled>인쇄(엑셀 다운)</button>
					</div>
				</div>
					...
					<div class="row mb-2">
						...
						<table class="table table-bordered" id="workOrdScanTable" style="margin-bottom:8px;">
							...
							<tr>
								...
								<td><input type="text" maxlength="6" min="0" max="100000" class="form-control  text-right touch6" id="appearOutputCnt" min="0" max="99999" disabled></td>
							...
							<tr>
								...
								<td style="text-align: right" id="badTotal4">0</td>
								<th class="touch6" id="badTotalNmae5">...</th>
								<td style="text-align: right" id="badTotal5">0</td>
							
								<!-- <td colspan="4" style="padding: 0px;">
									<div class="row col-sm-12 p-0">
										<div class="col-md-4 p-0">
											<span class="touch6 fakeTh" id="badTotalNmae4">...</span>
											<span class="fakeTd" id="badTotal4">0</span>
										</div>
										<div class="col-md-4 p-0">
							...
							<tr  id="visionTr">
							 	<th class="touch6">비전검사수량</th>
								<td ><input type="number" class="form-control text-right num_only touch6" id="visionInspCnt" disabled></td>
								
								<td ><input type="text" class="form-control touch6 d-none" id="note1" placeholder="원자재성 비고" disabled></td>
								<td ><input type="text" class="form-control touch6 d-none" id="note2" placeholder="공정성 비고" disabled></td>
							</tr>
...
<script>
	...
	let menuAuth = 'qmsc3320';
	let currentHref = 'qmsc3320';
	let currentPage = $('.'+currentHref).attr('id');
	$('#'+currentPage).closest('.has-child','li').addClass('has-open has-active');
	$('#'+currentPage).closest('.menu-item').addClass('has-active');
	$(document).attr("title","외관검사(6층)");
 ...
 */
	...
	for( var i=0; i<5; i++ ) {
		if( i < goodsFaultyBigCd.length ) {
	 		$('#tab'+parseInt(goodsFaultyBigCd[i].baseCd)).text(goodsFaultyBigCd[i].baseCdNm);
	 		$('#badTotalNmae'+parseInt(goodsFaultyBigCd[i].baseCd)).text(goodsFaultyBigCd[i].baseCdNm);
	     }
	
		if( $('#tab'+(i+1)).text() == "..." ) {
			console.log('#tab'+ (i+1) +'d-none 처리');
			$('#tab'+(i+1)).addClass('d-none');
			$('#badTotalNmae'+(i+1)).addClass('d-none');
			$('#badTotal'+(i+1)).addClass('d-none');
		}

		if(factoryCode != '003'){
			$('#visionTr').addClass('d-none');
...
// 	faultyHdCreate6();
	...
	let equipCodeAdmTable = $('#equipCodeAdmTable').on( 'error.dt', function ( e, settings, techNote, message ) {
        ...
        ajax: {
            ...
            data: {
            	'menuAuth'		:	menuAuth,
            	'equipGroup'	:	function() { return '002'; },
            },
        },
        rowId: 'equipCd',
		columns: [
    ...
    let outputCardTableSeq = $('#outputCardTableSeq').DataTable({
		...
		pageLength: 10,
        ajax: {
			url: '<c:url value="/qm/readGoodsFaultyAdmCodeList"/>',
			type: 'GET',
			async: false,
			data: {
				'menuAuth'			: 	menuAuth,
	        ...
	        success: function (res) {
	            ...
	            if (res.result == 'ok') {
					...
					//	//toastr.warning("해당 생산 LotNo는 외관검사 대상 생산 LotNo입니다. 생산LotNo를 확인해주세요!", '', {timeOut: 5000});
						...
						$('#userNm').val(data.appearInspctChargerNm);
						$('#userNumber').val(data.appearInspctCharger);
						$('#visionInspCnt').val(data.visionInspCnt);
						//$('#edgeOutputCnt').val(addCommas(data.edgeOutputCnt));
// 						$('#appearFaultyCnt').val(addCommas(data.appearFaultyCnt));
						//$('#ordLotNo').val(data.ordLotNo);
						//$('#workOrdNo').val(data.workOrdNo);
	...
	function sudel( dayNightCd, viewBoolean ) {
		$.ajax({
	        	url: '<c:url value="/qm/readTrayGoodsFaultyAdm_F2"/>',
	            type: 'GET',
	            data: {
	            	'menuAuth'	 	: 		menuAuth,
	            	'workOrdNo'		:		workOrdNo,		//작업지시
	            	'dayNightCd'	:		dayNightCd,		//작업지시
	            	'seq'			:		seq,		//작업지시
	            },
	            success: function (res) {
	                let data = res.data;
	                ...
	                if (res.result == 'ok') {
	                	...
	                	$.each($('.num_only'), function(index1,item1) {
	                		if(data.length > i) {
	                			$.each(data, function(index2,item2) {
		                			if ($(item1).parent().find('input[type=hidden][name=1]').val() == item2.minorCd) {
	                	...
	                	if( data != "" ) {
	                		...
	                		$('#appearOutputCnt').val( addCommas(data[0].appearOutputCnt) );
							$('#outputCnt').val( addCommas(data[0].userInputInspctCnt) );								
													
							$('#gubun').val(data[0].dayNightCd				== ""	|| data[0].dayNightCd		== null ? dayNightCd	: data[0].dayNightCd );
							$('#appearInspctDate').val(data[0].workOrdDate	== ""	|| data[0].workOrdDate		== null ? ordDate		: moment(data[0].workOrdDate).format('YYYY-MM-DD') );
							...
							$('#visionInspCnt').val(data[0].visionInspCnt	== "" 	|| data[0].visionInspCnt	== null ? ""	: data[0].visionInspCnt );
	...
	function totalDefectiveChange(){
		...
		for(var i=0; i<$('#metalFaultyTbody tr td').length; i++){
		    if($('#metalFaultyTbody tr td').eq(i).find('input').val() != 0){
		    	badTotal1 = parseInt(badTotal1) + parseInt($('#metalFaultyTbody tr td').eq(i).find('input').val());
		    }
		}
		for(var i=0; i<$('#filmFaultyDiv tr td').length; i++){
		    if($('#filmFaultyDiv tr td').eq(i).find('input').val() != 0){
		    	badTotal2 = parseInt(badTotal2) + parseInt($('#filmFaultyDiv tr td').eq(i).find('input').val());
		    }
		}
		for(var i=0; i<$('#visionFaultyDiv tr td').length; i++){
		    if($('#visionFaultyDiv tr td').eq(i).find('input').val() != 0){
		    	badTotal4 = parseInt(badTotal4) + parseInt($('#visionFaultyDiv tr td').eq(i).find('input').val());
		    }
		}
		...
		$('#badTotal4').text(addCommas(badTotal4));
		$('#badTotal5').text(addCommas(badTotal5));
// 		$('#badTotal6').text(addCommas(badTotal6));
		$('#appearFaultyCnt').val(addCommas(parseInt(badTotal1) + parseInt(badTotal2) + parseInt(badTotal4) + parseInt(badTotal5) ));
    }
	...
	$('#btnSave').on('click', function() {
       ...
       /*  if (parseInt(removeCommas($('#appearOutputCnt').val())) > 99999) {
		...
		//머티1,2동 불량 개수 100개 이상 알림처리
		if(factoryCode != '003'){
			$.each($('#metalFaultyTbody .num_only'), function(index1,item1) {
			...
			$.each($('#filmFaultyTbody .num_only'), function(index1,item1) {
				if($(this).val().replace(/,/g,"") != $(item1).parent().find('input[type=hidden][name=2]').val()){
					if(leftZeroRemove($(this).val()).length > 2){
						upperThanHunnit++;
					}
			...
			$.each($('#visionFaultyTbody .num_only'), function(index1,item1) {
				if($(this).val().replace(/,/g,"") != $(item1).parent().find('input[type=hidden][name=2]').val()){
					if(leftZeroRemove($(this).val()).length > 2){
						upperThanHunnit++;
					}
			...
			$.each($('#faultyTbody5 .num_only'), function(index1,item1) {
				if($(this).val().replace(/,/g,"") != $(item1).parent().find('input[type=hidden][name=2]').val()){
					if(leftZeroRemove($(this).val()).length > 2){
						upperThanHunnit++;
					}
	...
	function saveFaulty(){
		...
		$.each($('input.num_only'), function(index,item) {
			if (!$.trim($(this).val())) {
				if(factoryCode != '003'){ //1공장 2공장일때.
					if($(this).attr('id') != 'visionInspCnt'){
						$(this).focus();					
						numCheck = true;
						return false;
					}
        ...
        //if (trueFalse) {
			...
			$.each($('#metalFaultyTbody .num_only'), function() {
				if($(this).val() != '0' && $(this).val() != '00') {
					...
					rowData.visionInspCnt = $('#visionInspCnt').val().replace(/,/g,"");
					rowData.mainWorkChargr 	= workerId;		//	작업자
					rowData.teamCd 			= workerTeamCd;	//	작업조
					rowData.seq 			= seq;
					
					rowData.note1 			= $('#note1').val();
					rowData.note2			= $('#note2').val();
            ...
            var url = "";
     		if (sideView == 'add'){
    			url = '<c:url value="/qm/appearCheckInsert_F2"/>';
    		} else if (sideView == 'edit'){
     			url = '<c:url value="/qm/appearCheckUpdate_F2"/>';
     		}
...
// 		$('#gubun').attr('disabled',true);
	...
	$('#btnDeleteAppearCheck').on('click', function(){
		//삭제
		$.ajax({
			url : '<c:url value="qm/appearCheckDelete_F2"/>',
			type : 'POST',
			data : {
				'workOrdNo'		: function() { return workOrdNo;	},
...
// 		$('#badTotal6').text("0");
    ...
    function viewDisabled(flag){
    	...
    	$('#visionInspCnt').attr("disabled", flag);
    	//$("#gubun").attr("disabled", flag);
    	$('.num_only').attr('disabled', flag);
    	
    	$('#note1').attr('disabled', flag);
    	$('#note2').attr('disabled', flag);
    }
	...
	function etcFaultyHdCreate(){
    	...
    	$.each(systemCommonCode, function(index,item){				
    		if(item.etc1 == '003'){ 
    			...
    			Hd_metal2 += '<input type="hidden" id="etcCd'+arrayLength+'" name="1" value="'+item.baseCd+'">';
    			Hd_metal2 += '<input type="hidden" id="etcCd1'+arrayLength+'" name="2" value=""> </td>';
	...
	function visionFaultyHdCreate(){
    	...
    	$.each(systemCommonCode, function(index,item){
    		if(item.etc1 == '004'){ 
    			...
    			Hd_vision2 += '<input type="hidden" id="etcCd'+arrayLength+'" name="1" value="'+item.baseCd+'">';
    			Hd_vision2 += '<input type="hidden" id="etcCd1'+arrayLength+'" name="2" value=""> </td>';
	...
	function faultyHdCreate5(){
    	...
    	$.each(systemCommonCode, function(index,item){
    		if(item.etc1 == '005'){ 
    			...
    			Hd_5_2 += '<input type="hidden" id="etcCd'+arrayLength+'" name="1" value="'+item.baseCd+'"> </td>';
    			Hd_5_2 += '<input type="hidden" id="etcCd1'+arrayLength+'" name="2" value=""> </td>';
