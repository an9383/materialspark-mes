...
<div class="app pda_mo_wrap" style="display: flex; flex-direction: column; height: 100%;">
	...
	<div class="mo_sortbox">
		...
		<div class="col-12 row p-1">
			<div class="col-3 mo_sort_title">Tray No (3)</div>
			<div class="form-group input-sub col-9 m-0 p-0">
				<input type="text" class="form-control-md" id="boxNo" inputmode="none">
			</div>
		</div>
<!-- 		<div class="col-12 row p-1">
			<div class="col-3 mo_sort_title">Tray No (6)</div>
			<div class="form-group input-sub col-9 m-0 p-0">
				<input type="text" class="form-control-md" id="boxNo2" inputmode="none">
			</div>
		</div> -->
	</div>
	<!--==버튼영역-->
					...
					<table id="GoodsProductInfoTable" class="table table-bordered table-td-center">
						<thead class="thead-light">
							<tr>
								...
								<th style="min-width: 80px">TrayNo(3)</th>
								<th style="min-width: 50px">TrayNo(6)</th>
...
<script>
	...
	var lineCheck = false;
	$('#GoodsProductInfoTable tbody').on(
			'click',
			'tr',
			function() {
	...
	/* 				if ($('#boxNo').val().trim().length != "15") {
						toastr.warning('제품 바코드가 아닙니다. 확인해주세요');
						$('#boxNo').select();
						return false;
					} */

					var cval = compareBarcodeNo($('#boxNo').val(), 1);
					if (cval == false) {
					...
					$.ajax({
						url : '<c:url value="/pd/goodsProductInfoDataTray"/>',
						type : 'GET',
						data : {
							'menuAuth' : menuAuth,
						...
						success : function(res) {
							...
							if (res.result == 'ok') { //제품상태 포장, 제품입고
								...
								packCnt = data2[0].labelLotNo;
								remainCnt = data.remainCnt;
								locationCd = $("#location option:selected")
										.val();
								locationNm = $("#location option:selected")
										.text();
								$('#GoodsProductInfoTable').DataTable().row
										.add({}).draw(false);
								$('#btnSave').attr('disabled', false);
...
/* 					if ($('#boxNo2').val().trim().length != "11") {
						toastr.warning('제품 바코드가 아닙니다. 확인해주세요');
						$('#boxNo2').select();
						return false;
					} */

					var cval = compareBarcodeNo($('#boxNo2').val(), 2);
					if (cval == false) {
						toastr.warning('이미 추가된 바코드 입니다.<br/> 바코드정보를 확인해 주세요.',
								'', {
									timeOut : 5000
					...
					$.ajax({
						url : '<c:url value="/pd/goodsProductInfoDataTraySix"/>',
						type : 'GET',
						data : {
							'menuAuth' : menuAuth,
							'boxNo' : $('#boxNo2').val().trim(),
							'baseCd' : $("#location option:selected").val(),
							'restock' : restock,
						},
						beforeSend : function() {
						...
						success : function(res) {
							...
							if (res.result == 'ok') { //제품상태 포장, 제품입고
								...
								boxNo = data2.boxNo;
								goodsCd = data2.goodsCd;
								goodsNm = data2.goodsNm;
								packCnt = labelNo;
								remainCnt = data2.remainCnt;
								locationCd = $("#location option:selected").val();
								locationNm = $("#location option:selected").text();
								$('#GoodsProductInfoTable').DataTable().row.add({}).draw(false);
	...
	$('#btnSave').on('click', function() {
		...
		$.ajax({
			url : '<c:url value="/pd/updateProductLocationTray"/>',
			type : 'POST',
	...
	//중복 예외처리 사용예정
	function compareBarcodeNo(boxNo, type) {
		var result = true;
		...
		if(type == 1){
			...
			$('#GoodsProductInfoTable tbody tr').each(function() {
				var tboxNo = $(this).find('td').eq(4).html();
				if (boxNo == tboxNo) {
					result = false;
					return result;
				}
