...
<div class="app pda_mo_wrap" style="display: flex;flex-direction: column;height: 100%;">
    <header class="pda_mo_hd">
         ...
         <a href="/pdsc0040"><p class="mo_main_title">제품출고</p></a>
    </header>
	<!--====-sortbox-->
	<div class="mo_sortbox">
		...
		<div class="col-12 row p-1">
			<div class="col-3 mo_sort_title">Tray No</div>
			<div class="form-group input-sub col-9 m-0 p-0">
			<%
...
<%@include file="../layout/bottom.jsp" %> <!-- my-spinner 연결 -->

<script>

	$(document).attr("title","제품출고");
	let menuAuth = 'pdsc0040';
    ...
    $("#boxNo").keypress(function (e) {
        ...
        //new Promise((resolve, reject) => {
            if (e.which == 13){
                ...
                if ( !$.trim($('#boxNo').val()) ) {
                    toastr.warning('Tray No를 스캔해주세요!');
                    $("#boxNo").attr('disabled', false);
                    $('#boxNo').focus();
                    return false;
                }
                if ( ($('#boxNo').val().trim().length != "15")) {
                    toastr.warning('제품 바코드가 아닙니다. 확인해주세요'); 20220825-0056-1
                    $("#boxNo").attr('disabled', false);
                    $('#boxNo').select();
                    return false;
                }
                ...
                if(cval == false) {
                    toastr.warning('이미 추가된 Tray No 입니다.<br/> 트레이 정보를 확인해 주세요.', '', {timeOut: 5000});
                    $("#boxNo").attr('disabled', false);
                    $('#boxNo').select();
                    return false;
                }
							...
							if (data.goodsFifoCheckCd == "001") {
								if(res.fifoCheck == "yes") {
									...
									for(var i=0; i<res.checkList.length; i++){
										var rtn = '<tr><td style="text-align: center;">'+res.checkList[i].goodsNm+'</td>';
											rtn += '<td style="text-align: center;">'+res.checkList[i].boxNo+'</td>';
											rtn += '<td style="text-align: center;">'+res.checkList[i].goodsInDate+'</td>';
											rtn += '<td style="text-align: center;">'+res.checkList[i].remainCnt+'</td></tr>';
							...
							//선입선출 통제(확인)
								...
								if(res.fifoCheck == "yes") {
									...
									for(var i=0; i<res.checkList.length; i++){
										var rtn = '<tr><td style="text-align: center;">'+res.checkList[i].goodsNm+'</td>';
											rtn += '<td style="text-align: center;">'+res.checkList[i].boxNo+'</td>';
											rtn += '<td style="text-align: center;">'+moment(res.checkList[i].goodsInDate).format('YYYY-MM-DD');+'</td>';
											rtn += '<td style="text-align: center;">'+res.checkList[i].remainCnt+'</td></tr>';
										$('#goodsFifoCheckTable tbody').append(rtn);
									}
    ...
    $("#firstBoxNo").keypress(function (e) {
        ...
        //new Promise((resolve, reject) => {
            if (e.which == 13){
                ...
                if ( $('#firstBoxNo').val().trim().length != "15") {
                ...
                if(cval == false) {
                    toastr.warning('이미 추가된 Tray No 입니다.<br/> Tray 정보를 확인해 주세요.', '', {timeOut: 5000});
                    $("#firstBoxNo").attr('disabled', false);
                    $('#firstBoxNo').select();
                    return false;
                }
	...
	function createBoxNo() {
		...
		$('#boxCodeOutputWhsTable tbody tr').each(function(){
			...
			//var userOutputCnt	= removeCommas($(this).find('td input[name=userOutputCnt]').val());
		 	...
		 	var packCnt			= removeCommas($(this).find('td').eq(6).html());
			...
			rowData.customerLotNo 	= $(this).find('td').eq(8).text();
