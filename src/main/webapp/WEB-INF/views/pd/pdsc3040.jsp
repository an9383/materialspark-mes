...
<div class="app pda_mo_wrap" style="display: flex; flex-direction: column; height: 100%;">
	<header class="pda_mo_hd">
		<a href="/pdsc0070" class="float-left"><img class="p-2" src="/resources/assets/images/mo_home_wh.png"></a> <a href="/pdsc3040"><p class="mo_main_title">제품출고</p></a>
	</header>
	<!--====-sortbox-->
...
<script>
	...
	$('#btnSave').on('click', function() {
		...
		$.each(boxCodeOutputWhsTable.data(),function(idx, itm){		    
		    if(itm.disposeGubun == "003"){
		    	toastr.warning('폐기지정된 박스가 포함되어있습니다!');
		    	check = false;
				return false;
			}	
		});
	...
	$('#btnSaveCheck').on('click', function() {
		...
		if (createBoxNo()) {
			$.ajax({
				url : '<c:url value="/pd/palletRelatedProductOut_F3"/>',
				type : 'POST',
	...
	function selectPoCode() {
		var deliDateVal = $('#deliveryDate').val().replace(/-/g, '');
		if (poPopUpTable == null || poPopUpTable == undefined) {
			...
			poPopUpTable = $('#poPopUpTable').DataTable({
				...
				ajax : {
					...
					data : {
						'menuAuth' : menuAuth,
						'deliveryDate':	function() { return deliDateVal;	},
					},
				/*
				success : function(res) {
				    console.log(res);
			...
			$("#poPopUpTable_filter").parent().parent().children().eq(0).remove();
			$("#poPopUpTable_filter").parent().attr('class', 'col-sm-12 col-md-12');
			$("#poPopUpTable_filter").attr('style', 'margin-right:100% !important;');

			$('#deliveryDate').on('change', function(){
				deliDateVal = $('#deliveryDate').val().replace(/-/g, '');
				$('#poPopUpTable').DataTable().ajax.reload();
			});

			$('#allRetv').on('click', function(){
				deliDateVal = '';
				$('#poPopUpTable').DataTable().ajax.reload();
			});
	...
	function selectPoGroup() {
		...
		if (poGroupPopUpTable == null || poGroupPopUpTable == undefined) {
			...
			poGroupPopUpTable = $('#poGroupPopUpTable').DataTable({
				...
				ajax : {
					...
					data : {
						...
						'exceptPallet'	:	function() { return exceptPallet == "empty" ? '' : exceptPallet;		},
					},
				},
				rowId : 'poGroup',
				columns : [
					{ 
			    		render: function(data, type, row, meta) {    			
							return '<input type="checkbox" style="width:20px; height:20px; margin-top:2px;" class="chk" id="chkbox" name="chkbox" value="'+meta.row+'">' ;
			    		}
					},
					{ data : 'poGroup'		},
					{ data : 'outputQty'	},
				...
				order : [ [ 0, 'asc' ] ],
				rowCallback: function (row, data) {
					/* if (data.completeYn == 'Y' && data.goodsStatus == '004') {
			...
			$(document).on('click', '#chkbox', function(){
				if($('#poGroupPopUpTable').DataTable().data().length == $('.chk:checked').length){
					$('#checkAll').prop('checked', true);
				} else $('#checkAll').prop('checked', false);
			});

			//확정 버튼
			$('#poGroupSel').on('click', function(){
				if($(".chk:checked").length == 0){
					toastr.warning("파레트 번호를 선택해주세요!");
					return false;
				}

				//체크된 값들
				for(var j = 0; j < $(".chk:checked").length; j++) {
					
					var data = poGroupPopUpTable.row($($(".chk:checked")[j]).val()).data();

					$('#poGroup').val(data.poGroup);

					exceptPallet += "/" + "'" + data.poGroup + "'";
					
					sudel();

					chkCnt++;
					
					if(j == $(".chk:checked").length-1){
...
/*  			$('#poGroupPopUpTable tbody').on('click', 'tr', function() {
				...
				if( data.completeYn == "N" ) {
					toastr.warning("해당 파레트는 미료인 상태입니다.<br>완공 파레트를 선택해주세요!", '', {timeOut : 5000});
					return false;
				}
				
				if( data.goodsStatus == "003" ) {
					toastr.warning("해당 파레트는 출고 상태입니다.<br>출하대기 파레트를 선택해주세요!", '', {timeOut : 5000});
					return false;
				}
	...
	$("#poNum").keydown(function() {
		if(event.keyCode == 13){
			...
			if( comparePalletNo($('#poGroup').val().trim()) ) {
	...
	//중복 예외처리 사용예정
	function comparePalletNo(poGroup) {
		var result = false;
		for( var i=0; i<boxCodeOutputWhsTable.data().length; i++ ) {
			
			let data = boxCodeOutputWhsTable.data()[i];
			
			if( data.poGroup == poGroup ) {
				result = true;
			}
		}
		return result;
