  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
      	<button type="button" class="btn btn-success mr-2" id="btnMonthGoodsProductDownCheck">Excel 다운</button>
...
<script>
	...
	let goodsInventoryTable = $('#goodsInventoryTable').on( 'error.dt', function ( e, settings, techNote, message ) {
        ...
        pageLength: 20,
        ajax: {
            url: '<c:url value="/tm/tmByGoodsMonthInventoryList"/>',
            type: 'GET',
            data: {
            	'menuAuth'	 	: 		menuAuth,
    ...
    var html1 =  '<div class="row">&nbsp;<label class="input-label-sm">조회일자</label><input type="text" class="form-control" id="inWhsDateCal" style="width:65px;">';
	...
	//	html1 += '</div>';	
    	...
    	html1 += '<button type="button" class="btn btn-success mr-2" id="btnMonthGoodsProductDown">Excel 다운</button>';
    	html1 += '<div>';
	...
	let goodsInventoryDtlTable = $('#goodsInventoryDtlTable').on( 'error.dt', function ( e, settings, techNote, message ) {
        ...
        pageLength: 20,
        ajax: {
            url: '<c:url value="/tm/tmByGoodsMonthInventoryDtlList"/>',
            type: 'GET',
            data: {
            	'menuAuth'	 	: 		menuAuth,
	           	'packDate'		:		function() { return inWhsDateCal.replace(/-/g, ''); },	          
	...
	$('#btnMonthGoodsProductDown').on('click', function() {
		$('#excelDownLabel').text($('#inWhsDateCal').val() + ' 기준으로 모든 제품재고를 다운합니다.');
		$('#excelDownPopupModal').modal('show');
	});

	$('#btnMonthGoodsProductDownCheck').on('click', function() {
		$.ajax({
			url: '<c:url value="tm/monthGoodsProductionExcel"/>',
			type: 'GET',
			datatype: 'json',
			data:  {
				'menuAuth'	 	: 	menuAuth,
			...
			success: function (res) {
				...
				//console.log(data);
			    if (res.result == 'ok') {
			        location.href = '/tm/monthGoodsProductionExcelDownload?id=monthProduct_' + data.packDate;
			    } else if (res.result == 'fail') {
			    	toastr.warning(res.message);
			    } else if (res.result == 'error') {
					toastr.error("오류가 발생하였습니다. - monthExcel/download-001");
				}
			},
			complete:function(){
