				...
				<table class="table table-bordered p-0 m-0" id="bizOrdOutProdTable">
					<thead class="table-light">
						<tr>
							...
							<th class="text-center align-middle">사업장구분</th>
						</tr>
					</thead>
...
<script>
	...
	let bizOrdOutProdTable = $('#bizOrdOutProdTable').DataTable({
		dom : "<'row'<'col-md-12 col-md-6'l><'col-md-12 col-md-6'f>>"
				+ "<'row'<'col-md-12'tr>>"
				+ "<'row pt-1'B<'d-flex align-items-center d-flex'><'me-lg-auto'><'d-flex align-items-center justify-content-end'>>",
		language: lang_kor,
		info: true,
		ordering: true,
		processing: true,
        ...
        columns : [
			...
			{ data : 'packUnit', className : 'text-center align-middle', name : 'packUnit',
				render: function(data) {
					if(data != null && data != ''){
						return '<div style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">'+data+'</div>';
					} else {
			...
			{ data : 'bizStatusCdNm', className : 'text-center align-middle', name: 'bizStatusCdNm',
				render: function(data) {
					if(data != null && data != ''){
						return '<div style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">'+data+'</div>';
					} else {
			...
			{ data : 'outProdEtc2', className : 'text-center align-middle', name : '',
				render: function(data) {
					if(data != null && data != ''){
						return '<div style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">'+data+'</div>';
					} else {
			...
			{ data : 'outProdEtc3', className : 'text-center align-middle', name : '',
				render: function(data) {
					if(data != null && data != ''){
						return '<div style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">'+data+'</div>';
					} else {
		...
		drawCallback: function() {
			...
			$('#bizOrdOutProdTable_wrapper').find('.dataTables_scrollBody').css('height',(htmlHeight - theadHeight - 107)+'px');
			...
			$('#searchAllDtl').on('keyup',function() {
				$('#bizOrdOutProdTable_filter').find('input').val($(this).val());
				$('#bizOrdOutProdTable_filter').find('input').trigger('keyup');
			});
			...
			if( dataArray.length != 0 ){
				//맨밑에 최종합계
				let totalOutQtySum = _.sumBy(dataArray, v=> {
																if(isEmptyCheck(v.outQty)){
																	return 0;
																} else {
																	return parseFloat(v.outQty);
																}
															});
				let totalOrdQtySum = _.sumBy(dataArray, v=> {
																if(isEmptyCheck(v.bizOrdQty)){
																	return 0;
																} else {
																	return parseFloat(v.bizOrdQty);
																}
															});
