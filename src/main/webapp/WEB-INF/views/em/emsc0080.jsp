	...
	<header class="page-title-bar row">
		<nav aria-label="breadcrumb" class="breadcrumb-padding">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">계측기 이력</li>
			</ol>
		</nav>
	</header>
	<!-- #main============================================================== -->
				...
				<div class="card mb-2" id="formBox">
					...
					<div class="card-header card-tab p-0 mb-2">
						...
						<ul class="nav nav-tabs card-header-tabs m-0">
							<li class="nav-item">
								<a class="nav-link disabled d-none" id="tab1Nav" data-toggle="tab" href="#tab1">기본정보</a>
							</li>
							<li class="nav-item">
								<a class="nav-link active show" id="tab4Nav" data-toggle="tab" href="#tab4">이력조회</a>
							</li>
							<li class="nav-item">
								<a class="nav-link disabled d-none" id="tab3Nav" data-toggle="tab" href="#tab3">관련자료</a>
							</li>
						</ul>
						<!-- /.nav-tabs -->
					</div>
									...
									<table class="table table-lg table-bordered mb-2">
										...
										<thead>
											...
											<tr>
												...
												<th>관리책임자</th>
												<td><input type="text" maxlength="20" class="form-control" id="mainChargr" name="mainChargr" style="min-width:100%"></td>
											</tr>
											<tr>
												<th>*구분</th>
												<td>
													<select class="custom-select" id="mainGubun" name="mainGubun" style="min-width:100%"></select>
												</td>
												<th>*계측기명</th>
												<td>
...
<script>
	...
	<c:forEach items="${mitGubun}" var="sen"> //계측기구분
	var json = new Object();
...
/* 		$('#corrWillDate').attr('disabled', true); */
	...
	let measureInstrmtTable = $('#measureInstrmtTable').DataTable({
		...
		ajax : {
			...
			type : 'GET',
			data : {
				'mainGubun' : function(){return mainGubunList;},
...
//         }
	...
	selectBoxAppend(mainGubunArray, "mainGubun2", "", "1");
...
// 			$(this).removeClass('selected');
		...
		//선택 행 데이터 불러오기
		$.ajax({
			url : '<c:url value="/em/measureInstrmtAdmRead"/>',
			type : 'GET',
			data : {
			...
			success : function(res) {
				let data = res.data;
				imageFile1Nm = data.imageFile1Nm;
				imageFile1 = data.imageFile1;
				if(!(imageFile1 == "" || imageFile1 == null)){
					path = imageFile1.substring(0,imageFile1.indexOf("!"));
					changedFile1Nm = imageFile1.substring(imageFile1.indexOf("!")+1);
				}
				sideView = 'edit';
				...
				selectBoxAppend(mainGubunArray, "mainGubun", data.mainGubun, "");
				if (data.imageFile1 != '' && data.imageFile1 != null) {
					$('#imageFile1').attr("src",'/em/measureInstrmtAdmFileDownload?changedFile1Nm=' + changedFile1Nm +'&mitCdVal=' + mitCdVal);
		...
		//교정사항 이력
		$.ajax({
			url : '<c:url value="/em/measureInstrmtCorreAdmRead"/>',
			type : 'GET',
			data : {
				'mitCd' : function(){ return mitCdVal;}
			},
			success : function(res) {
				...
				if(data != null){
					...
					$('#fpHref').attr('href','/em/corrDataFileDownload?attachFn='+data.attachFn+'&mitCd='+mitCdVal);
...
*/
	...
	$('#btnDel').on('click', function() {
		...
		$.ajax({
			url : '<c:url value="/em/measureInstrmtAdmDelete"/>',
			type : 'POST',
			data : {
				'mitCd' : function(){return mitCdVal;},
				'attachGubun' : 'MI',
...
// 		if(sideView == 'add'){
// 			$.ajax({
// 				url : '<c:url value="em/itemPartDtlList"/>',
// 				type : 'GET',
// 				async : false,
// 				data : {
...
// 		}
		...
		if (sideView == "add" || editCheck) {
			$('#corrDataListTable tbody tr').each(function(index, item) {
				if ($(this).find("td input[name=corrNo]").val() != "" || $("#fileNm0").val() != "") {
...
// 					$(this).find("td input[name=corrDate]").focus();
		...
		var url = '<c:url value="/em/measureInstrmtAdmCreate"/>';

		$('#my-spinner').show();

		if (sideView == "edit") {
			url = '<c:url value="/em/measureInstrmtAdmUpdate"/>';
		}
		//기본등록 Ajax
		$.ajax({
			url : url,
			...
			success : function(res) {
				...
				if (res.result == 'ok') {
					...
					$.ajax({
						url : '<c:url value="/em/measureInstrmtImageUpload2"/>',
						data : formData,
						processData : false,
						contentType : false,
						type : 'POST',
...
// 		if(corrDateVal!=null && corrDateVal!="" && $('#corrDate').val().replace(/-/g,'') != corrDateVal){
// 			console.log("계측기 교정일자 등록 시작");

// 			var formData = new FormData(document.getElementById("formCorre"));
			
// 			var corrDate = $('#corrDate').val();	//교정일자
// 			var corrWillDate = $('#corrWillDate').val();	//교정일자
// 			var corrNo = $('#corrNo').val();		//교정번호
			
// 			formData.append('mitCd', mitCdVal);
// 			formData.append('corrSeq', corrSeqVal);
// 			formData.append('corrNo', corrNo);
// 			formData.append('corrDate', moment(corrDate).format('YYYYMMDD'));
// 			formData.append('corrWillDate', moment(corrWillDate).format('YYYYMMDD'));
...
// 			}, 
			{
				...
				render : function(data, type, row, meta) {
					if(data == null){
						...
						html += '<input class="form-control" type="date" id="corrDate_'+meta.row+'" value="'+tempCorrWillDate+'" name="corrDate"/>';
						html += '</div>';

						return html;
					}else{
						var html = '<div class="form-group input-sub m-0 row">';
						html += '<input class="form-control" type="date" id="corrDate_'+meta.row+'" value="'+moment(data).format('YYYY-MM-DD')+'" name="corrDate" disabled/>';
						html += '<input class="form-control" type="hidden" id="corrSeq_'+meta.row+'" value="'+row['corrSeq']+'" name="corrSeq"/>';
						html += '</div>';
			...
			}, 
			{
				...
				render : function(data, type, row, meta) {
					if(data == null){
						return '<input type="text" class="form-control" name="corrNo" style="width:100%"/>';
					}else{
						return '<input type="text" class="form-control" name="corrNo" value="'+data+'" style="width:100%" disabled/>';
					}
				},
				'className' : 'text-center'
			}, 
			{
				...
				render : function(data, type, row, meta) {
					...
					if(data == null){
						var html = '<form id="formFile_'+meta.row+'" method="post" enctype="multipart/form-data"> <div class="custom-file">';
						html += '<input type="file" class="custom-file-input" id="fileNm'+meta.row+'" name="fileNm">';
						html += '<label class="custom-file-label" for="fileNm'+meta.row+'"></label></div></form>';
						return html;
					} else {
						var html = '<a name="fileA" href="/em/corrDataFileDownload?attachFn='+data+'&mitCd='+mitCdVal+'&corrSeq='+row['corrSeq']+'">'+data+'</a>';
						html += '<form id="formFile_'+meta.row+'" method="post" enctype="multipart/form-data"> <div class="custom-file d-none" name="fileDiv">';
						html += '<input type="file" class="custom-file-input" id="fileNm'+meta.row+'" name="fileNm">';
						html += '<label class="custom-file-label" id="fileLabel'+meta.row+'" for="fileNm'+meta.row+'">'+data+'</label></div></form>';
						return html;
					}
	            }
			},
			...
			}, 
			{
				...
				render : function(data, type, row, meta) {
					...
					if(data == null){
						...
						var html = '<a name="" href="/em/corrDataFileDownload?attachFn='+data+'&mitCd='+mitCdVal+'&corrSeq='+row['corrSeq']+'&corrGubun='+row['corrGubun']+'">'+data+'</a>';
...
// 		}
	...
	$('#btnCorrDel').on('click', function() {
		...
		$.ajax({
			...
			data : {
				'mitCd' : function(){return mitCdVal;},
				'corrSeq' : function(){return corrSeqVal;},
			},
			success : function(res){
				if(res.result=="ok"){
					toastr.success("삭제되었습니다.");
					$('#corrDataListTable').DataTable().rows(tableIdx).remove().draw();
				}else {
					toastr.error(res.message);
				}
			}
...
//	var attachFn = "";
	...
	let measureInstrmAttachTable = $('#measureInstrmAttachTable').DataTable({
		...
		ajax : {
			url : '<c:url value="em/attachDataList"/>',
			type : 'GET',
			data : {
				'attachCd' : function() {return mitCdVal;},
				'attachGubun' : 'MI',
		...
		columns : [ 
			...
			}, 
			{
				...
				render : function(data, type, row) {
					...
					if(data == ''){
						del = '<a href="/em/measureAttachDataDownload?attachGubun=MI&attachFn='+encodeURIComponent(data)+'&attachSeq='+row['attachSeq']+'&attachCd='+mitCdVal+'&attachType=001">'+data+'</a>'
					} else{
						del = '<a href="/em/measureAttachDataDownload?attachGubun=MI&attachFn='+encodeURIComponent(data)+'&attachSeq='+row['attachSeq']+'&attachCd='+mitCdVal+'&attachType=001">'+data+'</a>' + del;
					}
...
/* 		success : function(res) {
			...
			attachFn = data.attachFn;
//			attachfileNm = attachCd + '_' + attachSeq + '_' + attachType + '.' + attachFn.substring(attachFn.indexOf("."));
...
//	status2 = '';
		...
		columns : [ 
			...
			}, 
			{
				...
				render : function(data, type, row) {
					...
					if(data == ''){
						del = '<a href="/em/measureAttachDataDownload?attachGubun=MI&attachFn='+encodeURIComponent(data)+'&attachSeq='+row['attachSeq']+'&attachCd='+mitCdVal+'&attachType=002">'+data+'</a>'
					} else{
						del = '<a href="/em/measureAttachDataDownload?attachGubun=MI&attachFn='+encodeURIComponent(data)+'&attachSeq='+row['attachSeq']+'&attachCd='+mitCdVal+'&attachType=002">'+data+'</a>' + del;
					}
	...
	$('#btnAttachDataAdd').on('click', function() {
		...
		$.ajax({
			url : '<c:url value="/em/attachDataSeq"/>',
			type : 'GET',
			data : {
				'attachGubun' : 'MI',
				'attachCd' : function(){return mitCdVal;}
...
// 		}
		...
		$.ajax({
			url : '<c:url value="/em/attachDataCreate"/>',
			type : 'POST',
			data : formData,
		    processData: false,
	        contentType: false,
...
// 		}
	...
	$('#btnAttachDataDelete').on('click', function() {
		...
		$.ajax({
			url : '<c:url value="/em/moldAttachFileDelete"/>',
			type : 'POST',
			data : {
				'attachGubun' : 'MI',
				'attachCd' : mitCdVal,
	...
	/* $('#btnAttachData2Delete').on('click', function() {
		...
		$.ajax({
			url : '<c:url value="/em/attachDataDelete"/>',
			type : 'POST',
			data : {
				'attachGubun' : 'MI',
				'attachCd' : mitCdVal,
	...
	let measureInstrmLogTable = $('#measureInstrmLogTable').DataTable({
		...
		columns : [ 
			...
			}, 
			{
				...
				render : function(data, type, row) {
					if(data == ''){
						del = '<a href="/em/corrDataFileDownload?attachFn='+data+'&mitCd='+mitCdVal+'">'+data+'</a>';
					} else{
						del = '<a href="/em/corrDataFileDownload?attachFn='+data+'&mitCd='+mitCdVal +'">'+data+'</a>';
					}
					return  del;
	...
	let inTable = $('#inTable').DataTable({
		...
		pageLength : 20,
		ajax : {
			url : '<c:url value="em/measureExcelDataList" />',
			type : 'GET',
			data : {
	...
	$('#btnExcelUpload').change( function() {
		...
		if (str.substring(str.lastIndexOf(".") + 1) == "xls"
			...
			$.ajax({
				...
				data : formData,
				url : '<c:url value="em/measureExcelUpload"/>',
				processData : false,
				contentType : false,
				cache : false,
				success : function(res) {
	...
	$('#btnApply').on('click',function(){
		...
		if(check){
			$.ajax({
				url : '<c:url value="em/measureExcelCreate"/>',
				type : 'POST',
				datatype: 'json',
	...
	function deleteImage() {
		...
		var path = imageFile1.substring(0,imageFile1.indexOf("!"));
		
		$.ajax({
			url : '<c:url value="/em/measureInstrmtfileDelete"/>',
			type : 'POST',
			data : {
					'path' : path,
					'imageFile1Nm' : imageFile1Nm
				   },
			success : function(res) {				
				if (res.result == 'ok') {
					toastr.success("파일 삭제");
				}
			}
		}) 
	}
