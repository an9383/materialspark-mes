		...
		<div class="row table-wrap-hid">
			...
			<div class="left-list" id="" style="width: 60%">
				<div class="card">
					<div class="row">
						<div class="row col-sm-9 mb-3 p-0">
							...
							<div class="row">
								...
								<input class="form-control ml-3 mr-3 touch6" type="text" id="barcodeWorkOrdLotNo" style="width:150px;" placeholder="생산LotNo 스캔">
							...
							<div class="ml-3 row float-right alert-primary">
								<h5 class="p-1 mr-3 mb-0">포장수량</h5>
								<h4 class="mb-0" id="packCnt"></h4>
							</div>
						</div>
						<div class="row col-sm-3">
							<button type="button" class="btn btn-primary float-right touch6" id="btnWorkOrdLotNoLineDelete">라인삭제</button>
					...
					<div class="col-md-12" style="overflow:scroll; padding:0px; min-height:150px; max-height:150px;">
				...
				<div class="card" id="formBox" style="margin-bottom: 0px;">
					<div class="card-body col-sm-12 pt-0">
						...
						%>
<!-- 						<div class="ml-5"> -->
							<input type="radio" class="float-right mr-5 mt-2" style="font-size:20px" name="paperSize" value="97_47">
							<label for="97_47" class="float-right mr-2" style="font-size:20px">구 버전</label>
							<input type="radio" class="float-right mr-2 mt-2" style="font-size:20px" name="paperSize" value="50_25">
							<label for="50_25" class="float-right mr-2" style="font-size:20px">신 버전</label>
<!-- 						</div> -->
						<%
							}
						%>
						<select class="custom-select custom-select-md ml-3 mr-5 float-left" style="width:120px" id="printYn">
							<option value="N">미발행</option>
							<option value="Y">재발행</option>
						</select>
						
						<button type="button" id="goodsBoxCardUp" class="btn btn-primary touch6 ml-5 mr-3">▲</button>
						<button type="button" id="goodsBoxCardDown" class="btn btn-primary touch6">▼</button>
					...
					<div id="scrollDiv" style="overflow:auto;">
						<div id="goodsBoxCard" class="col-12 row p-0 table-td-center" style="overflow:scroll; padding:0px; min-height:100px; max-height:500px;">
...
<div class="modal fade " id="packCancelCheckPopupModal" tabindex="-1" role="dialog" aria-labelledby="packCancelCheckPopupModalLabel" aria-hidden="true">
	<div class="modal-dialog" style="min-width:600px;">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<label for="goodsUnPackPrintCheck" id="goodsUnPackPrintCheckLabel" class="input-label-sm" style="font-size:20px; font-weight: bold;">인쇄라벨 복원 확인</label>
				<input type="checkbox" class="mr-5" id="goodsUnPackPrintCheck" style="zoom:2.5;">
...
<script>
	...
	let requestWorkOrdLotNo = "<%= request.getParameter("workOrdLotNo") %>";
	   
	history.replaceState({}, null, location.pathname);   //파라미터 값 초기화
	
	$(document).ready(function(){
	   if(requestWorkOrdLotNo != '' && requestWorkOrdLotNo != 'null'){
	      $('#barcodeWorkOrdLotNo').focus();
	      $('#barcodeWorkOrdLotNo').val(requestWorkOrdLotNo);
	      keypress('barcodeWorkOrdLotNo', '13');
	   }
	});
        	...
        	{ 
	    		...
	    		render: function(data, type, row, meta) {	
	    			if(data != null) {
						...
						return  $("#goodsNm").val() + '<input type="hidden" id="goodsCd" name="goodsCd" value="' + $("#goodsCd").val() + '">' + '<input type="hidden" id="packUnitCnt" name="packUnitCnt" value="' + $("#packUnitCnt").val() + '">'  + '<input type="hidden" id="packOrdLotNoList" name="packOrdLotNoList" value="' + $("#packOrdLotNoList").val() + '">';
	    			}
	    		}
    		},
        	{ 
...
// 	    				return  '<input type="text" class="form-control text-right userOutputCnt" style="min-width:40px max-width:50px" id="userOutputCnt" name="userOutputCnt" value="'+addCommas($("#userOutputCnt").val())+'">';
	    				return  '<input type="text" class="form-control text-right userOutputCnt" style="min-width:40px max-width:50px" id="userOutputCnt" name="userOutputCnt" value="'+addCommas(parseInt($("#totalInspectCnt").val()) - parseInt($("#totalFaultyCnt").val()))+'">';
						//return  addCommas($("#appearOutputCnt").val());
	    			}
	    		}
    		},
    		{ 
	...
	*/
	...
	function useGoodsPackSystem(type) {
		...
		if (type == 'goodsProductPack') {
			...
			if (goodsPackStatusCode == '001') {
				...
				//$('#btnSearch').trigger('click'); -> 포장 사용중일때(goodsUnPackStatusCode = 002) temp까지 날라가서 지워줬음
			}
	...
	//복원버튼 클릭 시
  	$('#btnPackCancel').on('click', function() {
		...
		goodsUnPackPrintCheck = false;

		$('#goodsUnPackPrintCheckLabel').addClass('d-none');
		$('#goodsUnPackPrintCheck').addClass('d-none');
		$('#goodsUnPackPrintCheck').prop('checked', false);
	...
	//복원모달 복원 클릭 시
  	$('#btnPackCancelCheck').on('click', function() {
		...
		if (goodsUnPackPrintCheck) {
			//복원대상 제품이 1번이라도 인쇄되었을 경우 확인 체크박스를 선택 하지 않았으면
			if (!$('#goodsUnPackPrintCheck').is(':checked')) {
				toastr.warning('복원 대상 포장번호 중 1번이라도 인쇄한 포장번호가 존재합니다!<br>한번 더 확인해 주세요!', '', {timeOut: 5000});
				$('#goodsUnPackPrintCheck').focus();
				return false;
			}			
		}
	...
	let boxNo1Table = $('#boxNo1Table').on( 'error.dt', function ( e, settings, techNote, message ) {
		...
		drawCallback: function (row, data, index) {
			...
			$('#boxNo1Table tbody tr td').css('height','30px');
        },
        //분할 상태
        rowCallback: function (row, data) {
			...
			} else if (parseInt(data.boxNo.substring(data.boxNo.length - 1, data.boxNo.length)) % 2 == 1) {
				$(row).addClass('whiteGray');
			}
        },
    });
	...
	let boxNo2Table = $('#boxNo2Table').on( 'error.dt', function ( e, settings, techNote, message ) {
		...
		drawCallback: function () {
            ...
            $('#boxNo2Table tbody tr td').css('height','30px');
        },
		//분할 상태
        rowCallback: function (row, data) {
			if (data.divisionYn == 'D') {
	...
	let boxNo3Table = $('#boxNo3Table').on( 'error.dt', function ( e, settings, techNote, message ) {
		...
		drawCallback: function () {
            ...
            $('#boxNo3Table tbody tr td').css('height','30px');
        },
		//분할 상태
        rowCallback: function (row, data) {
			if (data.divisionYn == 'D') {
	...
	$('#btnPrint3').on('click', function() {
		...
		if (factoryCode == '003' && $('input:radio[name=paperSize]:checked').val() == null) {
			toastr.warning('인쇄버전을 선택해주세요!')
			return false;
		}
		$('#my-spinner').show();
 ...
 */
	...
	*/
	...
	//제품 복원 시 프린트 여부확인
	let goodsUnPackPrintCheck = false;;
	...
	let checkUnPackListTable = $('#checkUnPackListTable').on( 'error.dt', function ( e, settings, techNote, message ) {
        ...
        rowCallback: function (row, data) {
			...
			if (data.printYn == 'Y') {
				$('#goodsUnPackPrintCheckLabel').removeClass("d-none");
				$('#goodsUnPackPrintCheck').removeClass("d-none");
				goodsUnPackPrintCheck = true;
			}
        },
    });
    ...
    */
	...
	$('#goodsBoxCardUp').on('click', function(){
		btnScrollTop()
	});

	$('#goodsBoxCardDown').on('click', function(){
		btnScrollBottom()
	});
	
	//상단 스크롤 이동 버튼 클릭 시
	function btnScrollTop() {
		let scrollTop = $('#goodsBoxCard').scrollTop();
    	$('#goodsBoxCard').scrollTop(scrollTop - 150);
	}

	//하단 스크롤 이동 버튼 클릭 시
    function btnScrollBottom() {
    	let scrollBottom = $('#goodsBoxCard').scrollTop();
    	$('#goodsBoxCard').scrollTop(scrollBottom + 150);
	}
	...
	function sendJson(printList) {
		if(factoryCode == '003') {
			paperSize = $("input:radio[name=paperSize]:checked").val();	
		}
		var obj = {
